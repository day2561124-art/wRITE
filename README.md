...
### 10.7. Full Workflow Smoke Test

Phase 6A 以隔離 fixture 串接完整流程：

`candidate_draft → proof_report → adopted_chapter → settlement_context → settlement_report → pending_engine_candidate → approval_queue → activation → rollback → cleanup_proposal → trash`

執行：

```powershell
node tests/e2e/full-workflow-smoke.test.mjs
```
