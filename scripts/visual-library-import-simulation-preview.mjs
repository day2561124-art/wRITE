#!/usr/bin/env node
import {
  compileVisualLibraryImportSimulationSummary,
...
function formatSummary(preview) {
  const summary = compileVisualLibraryImportSimulationSummary(preview);
  return [
    "Visual Library Import Simulation / Confirm Gate",
    `Phase: ${summary.phase}`,
    `Mode: ${summary.mode}`,
    `Source directory: ${summary.source_dir}`,
    `Confirmation accepted: ${summary.confirmation_accepted}`,
    `Decision: ${summary.decision}`,
    `Operations: ${summary.operation_count}`,
    `Simulated ready: ${summary.simulated_ready_count}`,
    `Blocked: ${summary.blocked_operation_count}`,
    `Rejected: ${summary.rejected_operation_count}`,
...
try {
  ...
  if (options.help) {
    console.log(
      "Usage: node scripts/visual-library-import-simulation-preview.mjs "
      + "[--source-dir <path>] [--confirm-text <text>] [--json|--pretty]",
    );
  } else {
    const preview = await runVisualLibraryImportSimulationPreview({
  ...
  console.error(`Visual library import simulation failed: ${error.message}`);
  process.exitCode = 1;
}
