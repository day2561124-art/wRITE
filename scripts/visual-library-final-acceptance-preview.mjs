...
function formatSummary(preview) {
  const summary = compileVisualLibraryFinalAcceptanceSummary(preview);
  return [
    "Visual Library Approval Guard UI Readiness / Final Acceptance Preview",
    `Phase: ${preview.phase}`,
    `Mode: ${preview.mode}`,
    ...
    `Dry-run ready: ${summary.ready_dry_run_case_count}`,
    `Blocked as expected: ${summary.blocked_as_expected_count}`,
    `Final acceptance decision: ${summary.final_acceptance_decision}`,
...
try {
  ...
  if (options.help) {
    console.log(
      "Usage: node scripts/visual-library-final-acceptance-preview.mjs "
      + "[--source-dir <path>] [--confirm-text <text>] [--json|--pretty]",
    );
  } else {
    const preview = await runVisualLibraryFinalAcceptancePreview({
  ...
  console.error(`Visual library final acceptance failed: ${error.message}`);
  process.exitCode = 1;
}
