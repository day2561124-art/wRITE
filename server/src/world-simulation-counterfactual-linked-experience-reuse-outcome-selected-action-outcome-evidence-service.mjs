...
export const worldSimulationCounterfactualLinkedExperienceReuseOutcomeSelectedActionOutcomeEvidenceVersion =
  "phase81q-counterfactual-linked-experience-reuse-outcome-selected-action-subjective-outcome-evidence-v1";
...
function requiredText(value, label, maxLength = 600) {
  ...
  if (!normalized || normalized.length > maxLength) {
    fail(
      "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_REUSE_OUTCOME_SELECTED_ACTION_OUTCOME_EVIDENCE_INPUT_INVALID",
      `Phase81Q ${label} must be a bounded non-empty string.`,
    );
  }
  return normalized;
}
...
function verifyExperience(experience, actionId) {
  if (!isObject(experience)
    ...
    fail(
      "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_REUSE_OUTCOME_SELECTED_ACTION_OUTCOME_EVIDENCE_PHASE76A_EXPERIENCE_INVALID",
      "Phase81Q requires the exact bounded scalar Phase76A subjective experience for the selected action.",
    );
  }
...
function verifyPhase76A(value, expectedTurnId) {
  ...
  if (!isObject(projection)
    ...
    fail(
      "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_REUSE_OUTCOME_SELECTED_ACTION_OUTCOME_EVIDENCE_PHASE76A_INVALID",
      "Phase81Q requires an exact canonical Phase76A subjective post-outcome projection for this turn.",
    );
  }
  ...
  if (boundaries.objective_world_outcome_remains_causal_authority !== true
    ...
    fail(
      "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_REUSE_OUTCOME_SELECTED_ACTION_OUTCOME_EVIDENCE_PHASE76A_BOUNDARY_INVALID",
      "Phase81Q rejects Phase76A evidence that violates the sealed subjective-observation boundary.",
    );
  }
  ...
  for (const experience of projection.character_experiences) {
    if (!isObject(experience)
      ...
      fail(
        "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_REUSE_OUTCOME_SELECTED_ACTION_OUTCOME_EVIDENCE_PHASE76A_EXPERIENCE_INVALID",
        "Phase81Q found an invalid Phase76A subjective experience record.",
      );
    }
    ...
    if (experience.subjective_perception_ref !== expectedRef
      ...
      fail(
        ...
        "Phase81Q Phase76A subjective experience identity or uniqueness verification failed.",
      );
    }
...
function exactPhase81PSources(expected, projection) {
  const sourceFields = [
  ...
  if (suppliedCount !== sourceFields.length) {
    fail(
      ...
      "Phase81Q exact Phase81P source validation requires Phase74D, Phase81N, Phase81O, and canonical prior World History together.",
    );
  }
  return {
...
export function buildWorldSimulationCounterfactualLinkedExperienceReuseOutcomeSelectedActionOutcomeEvidenceContract() {
  return Object.freeze({
    version: worldSimulationCounterfactualLinkedExperienceReuseOutcomeSelectedActionOutcomeEvidenceVersion,
    phase: "Phase81Q",
    status: "reuse_outcome_selected_action_subjective_outcome_evidence_installed",
    source_selected_action_lineage_owner: "Phase81P",
    source_subjective_outcome_owner: "Phase76A",
    exact_phase81p_receipt_bundle_hash_required: true,
    exact_phase81n_phase81o_phase74d_world_history_revalidation_required: true,
...
export function assertWorldSimulationCounterfactualLinkedExperienceReuseOutcomeSelectedActionOutcomeEvidence(
  value,
  expected = {},
) {
  const projection = cloneJson(value);
  if (!isObject(projection)
      || projection.version !== worldSimulationCounterfactualLinkedExperienceReuseOutcomeSelectedActionOutcomeEvidenceVersion
      || projection.phase !== "Phase81Q"
      ...
      || !text(projection.source_phase81p_receipt_bundle_hash)
      ...
      || hashWithout(projection, "projection_hash") !== projection.projection_hash) {
    fail(
      "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_REUSE_OUTCOME_SELECTED_ACTION_OUTCOME_EVIDENCE_INVALID",
      "Phase81Q reuse-outcome selected-action subjective outcome evidence is invalid.",
    );
  }
  for (const key of ["world_simulation_session_id", "turn_id", "state_revision", "world_state_hash"]) {
    if (Object.hasOwn(expected, key) && projection[key] !== expected[key]) {
      fail(
        ...
        `Phase81Q projection ${key} does not match expected lineage.`,
      );
    }
  }

  let expectedPhase81P = null;
  if (expected.counterfactual_linked_experience_reuse_outcome_selected_action_lineage !== undefined) {
    expectedPhase81P =
      assertWorldSimulationCounterfactualLinkedExperienceReuseOutcomeSelectedActionLineageBundle(
        expected.counterfactual_linked_experience_reuse_outcome_selected_action_lineage,
        exactPhase81PSources(expected, projection),
      );
    if (expectedPhase81P.version
          !== worldSimulationCounterfactualLinkedExperienceReuseOutcomeSelectedActionLineageVersion
        || projection.source_phase81p_receipt_bundle_hash
          !== expectedPhase81P.receipt_bundle_hash) {
      fail(
        "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_REUSE_OUTCOME_SELECTED_ACTION_OUTCOME_EVIDENCE_LINEAGE_MISMATCH",
        "Phase81Q no longer matches the exact canonical Phase81P selected-action lineage bundle.",
      );
    }
  }
  ...
  if (expected.post_outcome_subjective_perception_projection !== undefined) {
    ...
    if (projection.source_phase76a_projection_hash !== expectedPhase76A.projection_hash) {
      fail(
        "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_REUSE_OUTCOME_SELECTED_ACTION_OUTCOME_EVIDENCE_LINEAGE_MISMATCH",
        "Phase81Q no longer matches the exact Phase76A subjective outcome projection.",
      );
    }
  }

  const phase81PById = expectedPhase81P
    ? new Map(expectedPhase81P.receipts.map((receipt) => [receipt.receipt_id, receipt]))
    : null;
  ...
  for (const record of projection.evidence_records) {
    if (!isObject(record)
        || record.version !== worldSimulationCounterfactualLinkedExperienceReuseOutcomeSelectedActionOutcomeEvidenceVersion
        ...
        || !text(record.phase81p_receipt_id)
        || !text(record.phase81p_receipt_hash)
        || !text(record.phase81o_projection_hash)
        || !text(record.source_phase81n_projection_hash)
        ...
        || record.selection_relation !== "selected_action_matches_reuse_outcome_deliberative_intent"
        ...
        || record.prior_reuse_subjective_outcome_is_current_world_truth !== false
        ...
        || record.deliberative_reuse_intent_caused_selection_claimed !== false
        || record.reuse_effectiveness_inferred !== false
      ...
      fail(
        "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_REUSE_OUTCOME_SELECTED_ACTION_OUTCOME_EVIDENCE_RECORD_INVALID",
        "Phase81Q contains an invalid or over-authoritative subjective outcome evidence record.",
      );
    }
    verifyExperience(record.selected_action_subjective_experience, record.action_id);
    if (phase81PById && phase76AByRef) {
      const lineage = phase81PById.get(record.phase81p_receipt_id);
      const subjectivePerception = phase76AByRef.get(record.phase76a_subjective_perception_ref);
      if (!lineage
          || !subjectivePerception
          || lineage.receipt_hash !== record.phase81p_receipt_hash
          || lineage.phase81o_projection_hash !== record.phase81o_projection_hash
          || lineage.source_phase81n_projection_hash !== record.source_phase81n_projection_hash
        ...
        fail(
          "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_REUSE_OUTCOME_SELECTED_ACTION_OUTCOME_EVIDENCE_LINEAGE_MISMATCH",
          "Phase81Q evidence no longer matches its exact Phase81P selected-action lineage and Phase76A subjective outcome.",
        );
      }
    }
    ...
    const pairKey = `${record.phase81p_receipt_id}\n${record.phase76a_subjective_perception_ref}`;
    if (record.evidence_hash !== expectedHash
        || record.evidence_ref !== `phase81q_outcome_evidence_${expectedHash.slice(0, 24)}`
        || seenRefs.has(record.evidence_ref)
        || seenPairs.has(pairKey)) {
      fail(
        "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_REUSE_OUTCOME_SELECTED_ACTION_OUTCOME_EVIDENCE_RECORD_HASH_MISMATCH",
        "Phase81Q subjective outcome evidence identity verification failed.",
      );
    }
  ...
  if (projection.audit.exact_phase81p_selected_action_lineage_verified !== true
      || projection.audit.exact_phase81n_phase81o_phase74d_world_history_revalidated !== true
      ...
      || projection.audit.prior_reuse_subjective_outcome_is_current_world_truth !== false
      ...
      || projection.audit.deliberative_reuse_intent_caused_selection_claimed !== false
    ...
    fail(
      "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_REUSE_OUTCOME_SELECTED_ACTION_OUTCOME_EVIDENCE_BOUNDARY_INVALID",
      "Phase81Q projection violates its source-monitoring, authority, or persistence boundary.",
    );
  }
  return Object.freeze(projection);
}

export function buildWorldSimulationCounterfactualLinkedExperienceReuseOutcomeSelectedActionOutcomeEvidence(input = {}) {
  ...
  if (!Number.isSafeInteger(input.state_revision) || input.state_revision < 0) {
    fail(
      "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_REUSE_OUTCOME_SELECTED_ACTION_OUTCOME_EVIDENCE_INPUT_INVALID",
      "Phase81Q state_revision must be a non-negative safe integer.",
    );
  }
  ...
  if (phase81P.version
      !== worldSimulationCounterfactualLinkedExperienceReuseOutcomeSelectedActionLineageVersion) {
    fail(
      "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_REUSE_OUTCOME_SELECTED_ACTION_OUTCOME_EVIDENCE_PHASE81P_INVALID",
      "Phase81Q requires the canonical Phase81P selected-action lineage version.",
    );
  }
  ...
  for (const receipt of phase81P.receipts) {
    ...
    const identity = {
      ...
      phase81p_receipt_id: receipt.receipt_id,
      phase81p_receipt_hash: receipt.receipt_hash,
      phase81o_projection_hash: receipt.phase81o_projection_hash,
      source_phase81n_projection_hash: receipt.source_phase81n_projection_hash,
      ...
      source_phase81m_projection_hash: receipt.source_phase81m_projection_hash,
      source_phase81m_capsule_ref: receipt.source_phase81m_capsule_ref,
      source_phase81l_evidence_ref: receipt.source_phase81l_evidence_ref,
      reuse_intent_ref: receipt.reuse_intent_ref,
      retained_reuse_action_id: receipt.retained_reuse_action_id,
    ...
    evidenceRecords.push({
      evidence_ref: `phase81q_outcome_evidence_${evidenceHash.slice(0, 24)}`,
  ...
  if (evidenceRecords.length > maximumEvidenceRecordCount) {
    fail(
      "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_REUSE_OUTCOME_SELECTED_ACTION_OUTCOME_EVIDENCE_LIMIT_EXCEEDED",
      `Phase81Q accepts at most ${maximumEvidenceRecordCount} evidence records per turn.`,
    );
  }
  const projection = {
    version: worldSimulationCounterfactualLinkedExperienceReuseOutcomeSelectedActionOutcomeEvidenceVersion,
    ...
    state_revision: input.state_revision,
    world_state_hash: worldStateHash,
    source_phase81p_receipt_bundle_hash: phase81P.receipt_bundle_hash,
    source_phase76a_projection_hash: phase76A.projection_hash,
    evidence_count: evidenceRecords.length,
    evidence_records: evidenceRecords,
    audit: {
      exact_phase81p_selected_action_lineage_verified: true,
      exact_phase81n_phase81o_phase74d_world_history_revalidated: true,
