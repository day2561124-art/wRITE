...
async function main() {
  ...
  if (nameIdx === -1) {
    console.error('usage: node save-draft.mjs --name <name> [--file <path> | --content <text>]');
    process.exit(2);
  }
  ...
  // prevent accidental write to canon
  const banned = ['data/canon_db', 'data/canon_db/active_engine.md'];
  for (const b of banned) {
    if (name.includes(b) || (typeof content === 'string' && content.includes(b))) {
      console.error('Refusing: draft content or name references canon path');
      process.exit(3);
    }
  }

  const root = process.cwd();
  const draftsDir = join(root, 'outputs', 'drafts');
  await fs.mkdir(draftsDir, { recursive: true });
  const out = join(draftsDir, `${name}.md`);
  const meta = `---\ncreated_at: ${new Date().toISOString()}\nname: ${name}\n---\n\n`;
  await fs.writeFile(out, meta + content, 'utf8');
  console.log('saved draft to', out);
}

if (import.meta.url === `file://${process.argv[1]}` || process.argv[1] && process.argv[1].endsWith('save-draft.mjs')) {
