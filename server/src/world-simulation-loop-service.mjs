...
} from "./character-communication-repair-resolution-service.mjs";
import {
...
} from "./world-simulation-counterfactual-linked-experience-reuse-outcome-selected-action-lineage-service.mjs";
import {
...
} from "./world-simulation-counterfactual-linked-experience-longitudinal-case-selected-action-outcome-evidence-service.mjs";
import {
  buildWorldSimulationCounterfactualLinkedExperienceRetainedLongitudinalCaseOutcomeEvidence,
} from "./world-simulation-counterfactual-linked-experience-retained-longitudinal-case-outcome-evidence-service.mjs";
import {
...
export async function prepareWorldSimulationTurn(input = {}, options = {}) {
  ...
  const communicationRepairResolutionProjections = [];
  ...
  for (const character of participants) {
    ...
    if (listenerSpeechCandidates.length > 0) {
      characterPerception.information_boundary = {
        ...
        // CC-2 testimony admission remains intentionally closed until CC-6D
        // finds explicit same-listener speaker-identification evidence.
        listener_receipt_verified: false,
      };
    }

    const speakerRecognitionProjection =
      projectCharacterCommunicationSpeakerRecognition({
        observer: character,
        character_state: characterState,
        listener_understanding_projection: listenerUnderstandingProjection,
      });
    const understoodTestimonyReceipts =
      array(speakerRecognitionProjection.character_view?.understood_testimony_receipts);
    if (understoodTestimonyReceipts.length > 0) {
      characterPerception.audible = [
        ...array(characterPerception.audible),
        ...cloneJson(understoodTestimonyReceipts),
      ];
      characterPerception.information_boundary = {
        ...object(characterPerception.information_boundary),
        listener_receipt_verified: true,
        communication_listener_speaker_identity_recognized: true,
        communication_listener_speaker_identity_source:
          "same_listener_explicit_familiar_voice_evidence",
        communication_listener_testimony_subjective_only: true,
        communication_listener_world_truth_claimed: false,
        communication_listener_grounding_claimed: false,
      };
    }
    ...
    // repair initiator after current audibility + subjective interpretation.
    // Even an explicit "resolved" judgment is listener evidence only; it does
    // not establish mutual grounding or World truth.
    const repairResolutionAssembly =
      buildCharacterCommunicationRepairResolutionResolverView({
        observer: character,
        listener_understanding_projection: listenerUnderstandingProjection,
        character_state: characterState,
    ...
    const repairResolutionResolver =
      typeof options.characterCommunicationRepairResolutionResolver === "function"
        ? options.characterCommunicationRepairResolutionResolver
        : null;
    const rawRepairResolutionDecisions =
      repairResolutionResolver
        && repairResolutionAssembly.resolver_view.resolution_candidates.length > 0
        ? await repairResolutionResolver(
          cloneJson(repairResolutionAssembly.resolver_view),
        )
        : [];
    if (!Array.isArray(rawRepairResolutionDecisions)) {
      const error = new Error(
        "characterCommunicationRepairResolutionResolver must return an array of bounded listener resolution decisions.",
      );
      error.code =
        "WORLD_SIMULATION_COMMUNICATION_REPAIR_RESOLUTION_RESOLVER_INVALID_OUTPUT";
      throw error;
    }
    const repairResolutionProjection =
      projectCharacterCommunicationRepairResolution({
        assembly: repairResolutionAssembly,
        decisions: rawRepairResolutionDecisions,
      });
    const repairResolutionEvidence =
      array(repairResolutionProjection.character_view?.repair_resolution_evidence);
    if (repairResolutionEvidence.length > 0) {
      characterPerception.communication_repair_resolution_evidence =
        cloneJson(repairResolutionEvidence);
      characterPerception.information_boundary = {
        ...object(characterPerception.information_boundary),
        communication_repair_resolution_subjective_only: true,
        communication_repair_resolution_mutual_understanding_claimed: false,
        communication_repair_resolution_common_ground_updated: false,
        communication_repair_resolution_world_truth_claimed: false,
        communication_repair_resolution_grounding_claimed: false,
      };
    }
    communicationRepairResolutionProjections.push({
      character,
      version: characterCommunicationRepairResolutionVersion,
      candidate_count: repairResolutionProjection.audit.candidate_count,
      decision_count: repairResolutionProjection.audit.decision_count,
      evidence_count: repairResolutionProjection.audit.evidence_count,
      character_view_hash:
        hashAgentRunValue(repairResolutionProjection.character_view),
      audit: cloneJson(repairResolutionProjection.audit),
    ...
    const resolvedRepairSpeechCandidateIds = new Set(
      array(repairResolutionProjection.audit.resolved_speech_candidate_ids),
    );
    const repairInitiationResolverView =
      cloneJson(listenerUnderstandingAssembly.resolver_view);
    repairInitiationResolverView.speech_candidates =
      array(repairInitiationResolverView.speech_candidates)
        .filter((candidate) =>
          !resolvedRepairSpeechCandidateIds.has(candidate?.speech_candidate_id));

    // CC-6G adopts a same-listener CHOICE to request repair, not a World
    // communication action. A CC-6J-resolved response is omitted from this
    // turn's re-initiation opportunity; "still_trouble" remains eligible but
    // never auto-initiates another repair.
    ...
    const rawListenerRepairDecisions =
      listenerRepairResolver
        && repairInitiationResolverView.speech_candidates.length > 0
        ? await listenerRepairResolver(
          cloneJson(repairInitiationResolverView),
        )
        : [];
    ...
    communicationSpeakerRecognitionProjections.push({
      ...
      receipt_count:
        speakerRecognitionProjection.audit?.receipt_count ?? 0,
      character_view_hash:
        hashAgentRunValue(speakerRecognitionProjection.character_view),
      audit: cloneJson(speakerRecognitionProjection.audit),
      source_engine_identity_exposed_to_character: false,
      source_action_id_exposed_only_in_private_cc2_receipt:
        speakerRecognitionProjection.audit
          ?.source_action_id_exposed_only_in_private_cc2_receipt === true,
  ...
  return {
    ...
    communication_repair_resolution_projections:
      cloneJson(communicationRepairResolutionProjections),
    ...
    causal_boundary: {
      ...
      communication_speaker_recognition_version:
        characterCommunicationSpeakerRecognitionVersion,
      communication_speaker_recognition_requires_explicit_familiar_voice_identity: true,
      communication_speaker_recognition_relationship_field_not_used: true,
      communication_speaker_recognition_source_engine_identity_exposed: false,
      communication_speaker_recognition_testimony_content_from_listener_interpretation: true,
      ...
      communication_repair_resolution_version:
        characterCommunicationRepairResolutionVersion,
      communication_repair_resolution_current_audibility_required: true,
      communication_repair_resolution_committed_response_lineage_required: true,
      communication_repair_resolution_explicit_listener_attestation_required: true,
      communication_repair_resolution_resolved_suppresses_same_turn_reinitiation: true,
      communication_repair_resolution_still_trouble_auto_reinitiates: false,
      communication_repair_resolution_mutual_grounding_claimed: false,
...
export async function resolveWorldSimulationTurn(
  ...
  // Phase82F is accepted only as the exact same-turn Character Brain activation
  // over the canonical Phase82E cross-turn re-entry surface above. Resolve
  // revalidates the full Phase82E -> Phase82D/82C history lineage and current
  // Phase81N relevance so a locally rehashed activation cannot manufacture
  // effectiveness, preference, or action authority.
  const counterfactualLinkedExperienceLongitudinalCaseDeliberativeReuseProjections = [];
  const counterfactualLinkedExperienceLongitudinalCaseDeliberativeReuseInputProvided =
    Object.hasOwn(
      options,
      "counterfactualLinkedExperienceLongitudinalCaseDeliberativeReuseProjections",
    );
  const seenCounterfactualLinkedExperienceLongitudinalCaseDeliberativeReuseCharacters =
    new Set();
  for (const rawProjection of array(
    options.counterfactualLinkedExperienceLongitudinalCaseDeliberativeReuseProjections,
  )) {
    if (!counterfactualLinkedExperienceLongitudinalCaseReentry) {
      const error = new Error(
        "Phase82F projection requires the canonical Phase82E re-entry surface for this prepared turn.",
      );
      error.code =
        "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_LONGITUDINAL_CASE_REUSE_LINEAGE_INVALID";
      throw error;
    }
    const projection =
      assertWorldSimulationCounterfactualLinkedExperienceLongitudinalCaseDeliberativeReuseProjection(
        rawProjection,
        {
          character: rawProjection?.character,
          source_phase82e_projection:
    ...
    if (!allowedCounterfactualReflectionCharacters.has(projection.character)
        || seenCounterfactualLinkedExperienceLongitudinalCaseDeliberativeReuseCharacters
          .has(projection.character)) {
      const error = new Error(
        "Phase82F projections must map one-to-one to current prepared-turn characters.",
      );
      error.code =
        "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_LONGITUDINAL_CASE_REUSE_LINEAGE_INVALID";
      throw error;
    }
    seenCounterfactualLinkedExperienceLongitudinalCaseDeliberativeReuseCharacters.add(
      projection.character,
    );
    counterfactualLinkedExperienceLongitudinalCaseDeliberativeReuseProjections.push(
      cloneJson(projection),
    );
  }
  if (counterfactualLinkedExperienceLongitudinalCaseDeliberativeReuseInputProvided
      && seenCounterfactualLinkedExperienceLongitudinalCaseDeliberativeReuseCharacters.size
        !== allowedCounterfactualReflectionCharacters.size) {
    const error = new Error(
      "Phase82F internal input must provide exactly one projection for every current prepared-turn character.",
    );
    error.code =
      "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_LONGITUDINAL_CASE_REUSE_INCOMPLETE";
    throw error;
  }
  ...
  if (options.characterNativeTemporalResponseCausalRevalidationResolver !== undefined
      && (!nativeResponseEnabled
        || typeof options.characterNativeTemporalResponseCausalRevalidationResolver
          !== "function")) {
    const error = new Error(
      "CC-7AF causal revalidation requires the paired native response resolvers.",
    );
    error.code = "CC7AF_NATIVE_CAUSAL_REVALIDATION_RESOLVER_INVALID";
    throw error;
  }
  ...
  const nativeTemporalReplay = nativeObserver
    ? await replayWorldSimulationNativeTemporalResponse({
      ...
      causal_epoch_revalidation_resolver:
        options.characterNativeTemporalResponseCausalRevalidationResolver,
    })
    : null;
  ...
  // Phase81P closes only the deterministic provenance edge between a Phase81O
  // second-generation deliberative reuse intent present during this turn and
  // the exact Phase74D action then selected. Full Phase81N/81O canonical-history
  // lineage is revalidated; this records selection relation only and never
  // claims the reuse intent caused choice or establishes outcome/effectiveness.
  ...
  // and action. The complete Phase82E/82F/74D/current81N/history source chain is
  // revalidated. This observes experience only; effectiveness, success/failure,
  // counterfactual truth, causal/outcome credit, and cognitive/world revision are deferred.
  const counterfactualLinkedExperienceLongitudinalCaseSelectedActionOutcomeEvidence =
    counterfactualLinkedExperienceLongitudinalCaseSelectedActionLineage
      ? buildWorldSimulationCounterfactualLinkedExperienceLongitudinalCaseSelectedActionOutcomeEvidence({
        world_simulation_session_id: sessionId,
        turn_id: preparedTurn.turn_id,
        state_revision: snapshot.revision,
  ...
  // Phase82I assembles the current canonical Phase82H subjective outcome with
  // prior committed Phase82H outcomes for the exact same retained longitudinal
  // case identity. Every historical Phase82H source is replayed against its own
  // prior-history prefix. This is evidence assembly only: no effectiveness,
  // success/failure, reward/Q, causal credit, or cognitive/world revision occurs.
  const counterfactualLinkedExperienceRetainedLongitudinalCaseOutcomeEvidence =
    counterfactualLinkedExperienceLongitudinalCaseSelectedActionOutcomeEvidence
      ? buildWorldSimulationCounterfactualLinkedExperienceRetainedLongitudinalCaseOutcomeEvidence({
        world_simulation_session_id: sessionId,
        turn_id: preparedTurn.turn_id,
        state_revision: snapshot.revision,
        world_state_hash: snapshot.state_hash,
        world_history: counterfactualLinkedExperienceReuseOutcomeCanonicalHistory ?? [],
        current_phase82h_outcome_evidence:
          counterfactualLinkedExperienceLongitudinalCaseSelectedActionOutcomeEvidence,
        counterfactual_linked_experience_longitudinal_case_selected_action_lineage:
          counterfactualLinkedExperienceLongitudinalCaseSelectedActionLineage,
        subjective_choice_commitment_receipts:
  ...
  // Phase81Q joins only the exact Phase81P second-generation reuse selected-action
  // lineage with the bounded Phase76A subjective outcome for the same character
  // and action. The full Phase81N/81O/74D/history source chain is revalidated.
  // This records experienced outcome evidence only: it does not infer reuse
  // effectiveness, success/failure, causal credit, or any cognitive/world change.
  ...
  // Phase82A stops the single-case reuse loop from recursively cloning itself.
  // It assembles the current canonical Phase81Q subjective outcome together with
  // prior committed Phase81Q outcomes that share the same character and root
  // Phase81M reuse case. Every historical Q is revalidated against its own
  // P/N/O/74D/Phase76A sources and prior-history prefix. This remains evidence
  // assembly only: no effectiveness, success/failure, reward, causal credit, or
  // durable cognitive rule is inferred here.
  ...
  // that matched a Phase81J reuse intent, while revalidating the full canonical
  // Phase81I/81J/74D/history chain. It does not infer reuse effectiveness,
  // success/failure, historical counterfactual truth, causal credit, or any
  // belief/preference/memory/world-state revision.
  ...
  // causal outcomes or World State and may select only already-visible
  // alternative prospect branches plus a qualitative subjective comparison.
  // This evidence cannot revise the completed choice, assert what the unchosen
  // world would really have done, or assign regret/utility/Q-values.
  const postOutcomeCounterfactualAlternativeResolverView =
    buildWorldSimulationPostOutcomeCounterfactualAlternativeResolverView({
      world_simulation_session_id: sessionId,
      turn_id: preparedTurn.turn_id,
      state_revision: snapshot.revision,
  ...
  const postOutcomeCounterfactualAlternativeResolver =
    typeof options.postOutcomeCounterfactualAlternativeResolver === "function"
      ? options.postOutcomeCounterfactualAlternativeResolver
      : null;
  const rawPostOutcomeCounterfactualAlternativeDecisions =
    postOutcomeCounterfactualAlternativeResolver
      && postOutcomeCounterfactualAlternativeResolverView.counterfactual_context_count > 0
      ? await postOutcomeCounterfactualAlternativeResolver(
        cloneJson(postOutcomeCounterfactualAlternativeResolverView),
      )
      : [];
  if (!Array.isArray(rawPostOutcomeCounterfactualAlternativeDecisions)) {
    const error = new Error(
      "postOutcomeCounterfactualAlternativeResolver must return an array of bounded Phase81A comparison decisions.",
    );
    error.code =
      "WORLD_SIMULATION_POST_OUTCOME_COUNTERFACTUAL_RESOLVER_INVALID_OUTPUT";
    throw error;
  }
  const postOutcomeCounterfactualAlternativeEvidence =
    projectWorldSimulationPostOutcomeCounterfactualAlternativeEvidence({
      world_simulation_session_id: sessionId,
      turn_id: preparedTurn.turn_id,
      state_revision: snapshot.revision,
      ...
      post_outcome_subjective_perception_projection:
        postOutcomeSubjectivePerceptionProjection,
      resolver_view: postOutcomeCounterfactualAlternativeResolverView,
  ...
  const committed = await commitWorldSimulationTurn(
    sessionId,
    {
      ...
      communication_repair_resolution_projections: cloneJson(
        preparedTurn.communication_repair_resolution_projections ?? [],
      ),
      ...
      counterfactual_linked_experience_longitudinal_case_selected_action_outcome_evidence:
        cloneJson(counterfactualLinkedExperienceLongitudinalCaseSelectedActionOutcomeEvidence),
      counterfactual_linked_experience_retained_longitudinal_case_outcome_evidence:
        cloneJson(counterfactualLinkedExperienceRetainedLongitudinalCaseOutcomeEvidence),
      counterfactual_linked_experience_reuse_outcome_deliberation_projections:
        cloneJson(counterfactualLinkedExperienceReuseOutcomeDeliberationProjections),
      counterfactual_linked_experience_reuse_outcome_selected_action_lineage:
        cloneJson(counterfactualLinkedExperienceReuseOutcomeSelectedActionLineage),
  ...
  return {
    ...
    post_outcome_counterfactual_alternative_evidence: {
      ...
      eligible_context_count:
        postOutcomeCounterfactualAlternativeResolverView.counterfactual_context_count,
      evidence_count:
        postOutcomeCounterfactualAlternativeEvidence.counterfactual_evidence_count,
      projection_hash:
        postOutcomeCounterfactualAlternativeEvidence.projection_hash,
