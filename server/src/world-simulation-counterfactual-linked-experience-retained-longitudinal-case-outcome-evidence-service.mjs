...
export const worldSimulationCounterfactualLinkedExperienceRetainedLongitudinalCaseOutcomeEvidenceVersion =
  "phase82i-counterfactual-linked-experience-retained-longitudinal-case-outcome-evidence-v1";
...
function requiredText(value, label, maxLength = 600) {
  ...
  if (!normalized || normalized.length > maxLength) {
    fail(
      "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_RETAINED_LONGITUDINAL_CASE_OUTCOME_EVIDENCE_INPUT_INVALID",
      `Phase82I ${label} must be a bounded non-empty string.`,
    );
  }
  return normalized;
}
...
function historyPrefix(worldHistory, endIndex) {
  const history = isObject(worldHistory)
    ? cloneJson(worldHistory)
    : { turns: cloneJson(turns(worldHistory)) };
  history.turns = turns(worldHistory).slice(0, endIndex).map(cloneJson);
  return history;
}
function expectedPhase82HFromTurn(worldHistory, turn, index, sessionId) {
  return {
    ...
    counterfactual_linked_experience_longitudinal_case_selected_action_lineage:
      turn.counterfactual_linked_experience_longitudinal_case_selected_action_lineage,
    ...
    counterfactual_linked_experience_longitudinal_case_reentry:
      turn.counterfactual_linked_experience_longitudinal_case_reentry,
    counterfactual_linked_experience_longitudinal_case_deliberative_reuse_projections:
      turn.counterfactual_linked_experience_longitudinal_case_deliberative_reuse_projections,
    ...
    world_history: historyPrefix(worldHistory, index),
    post_outcome_subjective_perception_projection:
      turn.post_outcome_subjective_perception_projection,
  };
}
function canonicalPriorPhase82H(worldHistory, turn, index, sessionId) {
  const projection =
    turn?.counterfactual_linked_experience_longitudinal_case_selected_action_outcome_evidence;
  ...
  return assertWorldSimulationCounterfactualLinkedExperienceLongitudinalCaseSelectedActionOutcomeEvidence(
    projection,
    expectedPhase82HFromTurn(worldHistory, turn, index, sessionId),
  );
}
function expectedCurrentPhase82H(input, identity) {
  return {
    world_simulation_session_id: identity.world_simulation_session_id,
...
function boundedEvidenceItem(record, role) {
  return {
    ...
    phase82h_evidence_ref: record.evidence_ref,
    phase82h_evidence_hash: record.evidence_hash,
    source_phase82d_admission_ref: record.source_phase82d_admission_ref,
    source_phase82d_admission_hash: record.source_phase82d_admission_hash,
    source_phase82c_appraisal_ref: record.source_phase82c_appraisal_ref,
    source_phase82c_appraisal_hash: record.source_phase82c_appraisal_hash,
    ...
    selected_action_subjective_experience:
      cloneJson(record.selected_action_subjective_experience),
    subjective_not_world_truth: true,
  };
}
function sameCase(left, right) {
  return characterKey(left.character) === characterKey(right.character)
    && left.source_phase82d_admission_ref === right.source_phase82d_admission_ref
    && left.source_phase82d_admission_hash === right.source_phase82d_admission_hash
    && left.source_phase81m_capsule_ref === right.source_phase81m_capsule_ref
    && left.reuse_intent_ref === right.reuse_intent_ref;
}
function canonicalPriorRecords(worldHistory, sessionId) {
  const result = [];
  const historyTurns = turns(worldHistory);
  for (let index = 0; index < historyTurns.length; index += 1) {
    const turn = historyTurns[index];
    if (!isObject(turn)) continue;
    const projection = canonicalPriorPhase82H(worldHistory, turn, index, sessionId);
    if (!projection) continue;
    result.push({ turn_index: index, projection });
  }
  return result;
}
function expectedPriorItems(currentRecord, canonicalPrior) {
  const priorItems = [];
  for (let index = canonicalPrior.length - 1; index >= 0; index -= 1) {
    const priorProjection = canonicalPrior[index].projection;
    for (const priorRecord of priorProjection.evidence_records) {
      if (!sameCase(currentRecord, priorRecord)) continue;
      priorItems.push(boundedEvidenceItem(priorRecord, "prior_retained_case_outcome"));
...
export function buildWorldSimulationCounterfactualLinkedExperienceRetainedLongitudinalCaseOutcomeEvidenceContract() {
  return Object.freeze({
    version: worldSimulationCounterfactualLinkedExperienceRetainedLongitudinalCaseOutcomeEvidenceVersion,
    phase: "Phase82I",
    status: "bounded_retained_longitudinal_case_outcome_evidence_assembly_installed",
    source_current_outcome_owner: "Phase82H",
    source_prior_outcome_owner: "Phase82H committed World History",
    exact_current_phase82h_lineage_required: true,
    exact_prior_phase82h_per_turn_revalidation_required: true,
    same_character_same_phase82d_admission_required: true,
    same_phase81m_capsule_and_reuse_intent_required: true,
...
export function assertWorldSimulationCounterfactualLinkedExperienceRetainedLongitudinalCaseOutcomeEvidence(
  value,
  expected = {},
) {
  const projection = cloneJson(value);
  if (!isObject(projection)
      || projection.version !== worldSimulationCounterfactualLinkedExperienceRetainedLongitudinalCaseOutcomeEvidenceVersion
      || projection.phase !== "Phase82I"
      ...
      || !text(projection.source_current_phase82h_projection_hash)
    ...
    fail(
      "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_RETAINED_LONGITUDINAL_CASE_OUTCOME_EVIDENCE_INVALID",
      "Phase82I retained longitudinal-case outcome evidence projection is invalid.",
    );
  }
  for (const key of ["world_simulation_session_id", "turn_id", "state_revision", "world_state_hash"]) {
    if (Object.hasOwn(expected, key)
      ...
      fail(
        "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_RETAINED_LONGITUDINAL_CASE_OUTCOME_EVIDENCE_LINEAGE_MISMATCH",
        `Phase82I projection ${key} does not match expected lineage.`,
      );
    }
  }
  ...
  for (const context of projection.character_case_contexts) {
    if (!isObject(context)
        ...
        || !text(context.source_phase82d_admission_ref)
        || !text(context.source_phase82d_admission_hash)
        ...
        || !Array.isArray(context.prior_retained_case_outcomes)
        || context.prior_retained_case_outcome_count !== context.prior_retained_case_outcomes.length
        || context.prior_retained_case_outcome_count > maximumPriorOutcomeCountPerAnchor
        || context.longitudinal_ready !== (context.prior_retained_case_outcome_count > 0)
        || context.retained_admission_is_effectiveness_claim !== false
        || context.retained_admission_is_success_claim !== false
      ...
      fail(
        "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_RETAINED_LONGITUDINAL_CASE_OUTCOME_EVIDENCE_CONTEXT_INVALID",
        "Phase82I contains an invalid or over-authoritative longitudinal context.",
      );
    }
    ...
    for (const key of [
      ...
      "effectiveness_interpretation_performed",
    ...
    if (context.context_hash !== expectedHash
        || context.context_ref !== `phase82i_retained_longitudinal_${expectedHash.slice(0, 24)}`
        || seenContextRefs.has(context.context_ref)) {
      fail(
        "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_RETAINED_LONGITUDINAL_CASE_OUTCOME_EVIDENCE_CONTEXT_HASH_MISMATCH",
        "Phase82I retained longitudinal context identity verification failed.",
      );
    }
    seenContextRefs.add(context.context_ref);
  }

  const hasCanonicalExpected = expected.current_phase82h_outcome_evidence !== undefined
    || expected.world_history !== undefined;
  if (hasCanonicalExpected) {
    const required = [
      expected.current_phase82h_outcome_evidence,
    ...
    if (required.some((item) => item === undefined)) {
      fail(
        "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_RETAINED_LONGITUDINAL_CASE_OUTCOME_EVIDENCE_EXPECTED_SOURCE_SET_INCOMPLETE",
        "Phase82I canonical validation requires current Phase82H sources and canonical prior World History together.",
      );
    }
    const currentH =
      assertWorldSimulationCounterfactualLinkedExperienceLongitudinalCaseSelectedActionOutcomeEvidence(
        expected.current_phase82h_outcome_evidence,
        expectedCurrentPhase82H(expected, projection),
      );
    if (currentH.version
          !== worldSimulationCounterfactualLinkedExperienceLongitudinalCaseSelectedActionOutcomeEvidenceVersion
        || currentH.projection_hash !== projection.source_current_phase82h_projection_hash) {
      fail(
        "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_RETAINED_LONGITUDINAL_CASE_OUTCOME_EVIDENCE_LINEAGE_MISMATCH",
        "Phase82I no longer matches the exact canonical current Phase82H projection.",
      );
    }
    const currentByRef = new Map(
      currentH.evidence_records.map((record) => [record.evidence_ref, record]),
    );
    const canonicalPrior = canonicalPriorRecords(
      expected.world_history,
      projection.world_simulation_session_id,
    );
    for (const context of projection.character_case_contexts) {
      const currentRecord = currentByRef.get(context.current_anchor.phase82h_evidence_ref);
      if (!currentRecord
          || currentRecord.evidence_hash !== context.current_anchor.phase82h_evidence_hash
          || !sameCase(currentRecord, context)
          || hashAgentRunValue(boundedEvidenceItem(currentRecord, "current_anchor"))
            !== hashAgentRunValue(context.current_anchor)) {
        fail(
          "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_RETAINED_LONGITUDINAL_CASE_OUTCOME_EVIDENCE_LINEAGE_MISMATCH",
          "Phase82I current anchor no longer matches exact current Phase82H evidence.",
        );
      }
      const expectedPrior = expectedPriorItems(currentRecord, canonicalPrior);
      if (hashAgentRunValue(expectedPrior)
          !== hashAgentRunValue(context.prior_retained_case_outcomes)) {
        fail(
          "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_RETAINED_LONGITUDINAL_CASE_OUTCOME_EVIDENCE_LINEAGE_MISMATCH",
          "Phase82I prior retained-case outcomes no longer match canonical Phase82H history.",
        );
      }
    }
  }

  if (projection.audit.exact_current_phase82h_lineage_verified !== true
      || projection.audit.exact_prior_phase82h_per_turn_revalidation_performed !== true
      || projection.audit.same_character_same_phase82d_admission_required !== true
      || projection.audit.same_phase81m_capsule_and_reuse_intent_required !== true
      || projection.audit.individual_outcome_specificity_preserved !== true
    ...
    fail(
      "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_RETAINED_LONGITUDINAL_CASE_OUTCOME_EVIDENCE_BOUNDARY_INVALID",
      "Phase82I projection violates its evidence-only authority boundary.",
    );
  }
  return Object.freeze(projection);
}

export function buildWorldSimulationCounterfactualLinkedExperienceRetainedLongitudinalCaseOutcomeEvidence(
  input = {},
) {
  ...
  if (!Number.isSafeInteger(input.state_revision) || input.state_revision < 0) {
    fail(
      "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_RETAINED_LONGITUDINAL_CASE_OUTCOME_EVIDENCE_INPUT_INVALID",
      "Phase82I state_revision must be a non-negative safe integer.",
    );
  }
  ...
  const currentH =
    assertWorldSimulationCounterfactualLinkedExperienceLongitudinalCaseSelectedActionOutcomeEvidence(
      input.current_phase82h_outcome_evidence,
      expectedCurrentPhase82H(input, identity),
    );
  if (currentH.version
      !== worldSimulationCounterfactualLinkedExperienceLongitudinalCaseSelectedActionOutcomeEvidenceVersion) {
    fail(
      "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_RETAINED_LONGITUDINAL_CASE_OUTCOME_EVIDENCE_PHASE82H_INVALID",
      "Phase82I requires the canonical Phase82H outcome-evidence version.",
    );
  }

  const canonicalPrior = canonicalPriorRecords(input.world_history, worldSimulationSessionId);
  const contexts = [];
  for (const currentRecord of currentH.evidence_records) {
    const priorItems = expectedPriorItems(currentRecord, canonicalPrior);
    const contextIdentity = {
      ...
      source_phase82d_admission_ref: currentRecord.source_phase82d_admission_ref,
      source_phase82d_admission_hash: currentRecord.source_phase82d_admission_hash,
      ...
      prior_retained_case_outcomes: priorItems,
      prior_retained_case_outcome_count: priorItems.length,
      longitudinal_ready: priorItems.length > 0,
    };
    const contextHash = hashAgentRunValue(contextIdentity);
    contexts.push({
      context_ref: `phase82i_retained_longitudinal_${contextHash.slice(0, 24)}`,
  ...
  if (contexts.length > maximumContextCount) {
    fail(
      "WORLD_SIMULATION_COUNTERFACTUAL_LINKED_RETAINED_LONGITUDINAL_CASE_OUTCOME_EVIDENCE_LIMIT_EXCEEDED",
      `Phase82I accepts at most ${maximumContextCount} current anchor contexts per turn.`,
    );
  }
  const projection = {
    version: worldSimulationCounterfactualLinkedExperienceRetainedLongitudinalCaseOutcomeEvidenceVersion,
    ...
    state_revision: input.state_revision,
    world_state_hash: worldStateHash,
    source_current_phase82h_projection_hash: currentH.projection_hash,
    context_count: contexts.length,
    longitudinal_ready_context_count:
      contexts.filter((context) => context.longitudinal_ready).length,
    character_case_contexts: contexts,
    audit: {
      exact_current_phase82h_lineage_verified: true,
      exact_prior_phase82h_per_turn_revalidation_performed: true,
      prior_phase82h_projection_count_verified: canonicalPrior.length,
      same_character_same_phase82d_admission_required: true,
      same_phase81m_capsule_and_reuse_intent_required: true,
      individual_outcome_specificity_preserved: true,
      prior_and_current_subjective_outcomes_source_distinct: true,
  ...
  return assertWorldSimulationCounterfactualLinkedExperienceRetainedLongitudinalCaseOutcomeEvidence(
    projection,
    {
      ...identity,
      current_phase82h_outcome_evidence: currentH,
      world_history: input.world_history,
      counterfactual_linked_experience_longitudinal_case_selected_action_lineage:
        input.counterfactual_linked_experience_longitudinal_case_selected_action_lineage,
      subjective_choice_commitment_receipts:
