...
const toolDefinitions = [
  ...
  },
  {
    ...
    description: "[low-risk-write] Begin a ChatGPT-owned world-simulation cognition session. This route does not build a formal writing context and does not mutate Canon, active_engine, or world state. The programmatic simulator remains the authority for hard state and causal outcomes.",
    risk: "low-risk-write",
    inputSchema: baseSchema({
  ...
  },
  {
    ...
    description: "[low-risk-write] Normalize one Scene State into causal geometry and simultaneous-action adjudication inputs. It prepares evidence only and never decides success, hit, injury, destruction, or victory.",
    risk: "low-risk-write",
  ...
  },
  {
    ...
    description: "[low-risk-write] Build one character's bounded perception packet from explicit sensory observations and observer-scoped Scene State. Hidden fields, private device contents, and other characters' internal states are excluded.",
    risk: "low-risk-write",
    inputSchema: baseSchema({ world_simulation_session_id: { type: "string" }, capability_input: { type: "object" } }, ["world_simulation_session_id", "capability_input"]),
  ...
  },
  {
    ...
    description: "[low-risk-write] Assemble one named character's bounded cognition from perception, memory, needs, emotion, values, relationships, attention, goals, and current action. It cannot choose a world result.",
    risk: "low-risk-write",
    inputSchema: baseSchema({ world_simulation_session_id: { type: "string" }, capability_input: { type: "object" } }, ["world_simulation_session_id", "capability_input"]),
  ...
  },
  {
    ...
    description: "[low-risk-write] Return non-binding character action intents from simulator-supplied available actions. ChatGPT's character brain chooses the intent; the causal simulator decides the result.",
    risk: "low-risk-write",
    inputSchema: baseSchema({ world_simulation_session_id: { type: "string" }, capability_input: { type: "object" } }, ["world_simulation_session_id", "capability_input"]),
  ...
  },
  {
    ...
    description: "[low-risk-write] Detect narrative, camera, spotlight, romance, or governance control signals that would improperly choose a character's action or make observation create causality.",
    risk: "low-risk-write",
    inputSchema: baseSchema({ world_simulation_session_id: { type: "string" }, capability_input: { type: "object" } }, ["world_simulation_session_id", "capability_input"]),
  ...
  },
  {
    ...
    description: "[low-risk-write] Diagnose explicit world-state transitions for missing causes, duplicate exclusive ownership, knowledge gain without source, and outcome-first shortcuts. It reports only and never repairs history automatically.",
    risk: "low-risk-write",
    inputSchema: baseSchema({ world_simulation_session_id: { type: "string" }, capability_input: { type: "object" } }, ["world_simulation_session_id", "capability_input"]),
...
const permissionSources = {
  ...
  chatgpt_bridge_begin_world_simulation_session: ["user_input", "agent_run_records"],
