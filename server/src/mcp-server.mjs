...
import {
  ...
  DEV_WORKSPACE_TYPES,
  DEV_WORKSTREAM_LIST_LIFECYCLES,
  DEV_WORKSTREAM_MAX_DEPENDENCIES,
  DEV_WORKSTREAM_MAX_LABEL_CHARACTERS,
  DEV_WORKSTREAM_MAX_LIST_RESULTS,
  DEV_WORKSTREAM_MAX_METADATA_PROPERTIES,
  DEV_WORKSTREAM_MAX_METADATA_STRING_CHARACTERS,
  DEV_WORKSTREAM_MAX_SCOPE_ENTRIES,
  DEV_WORKSTREAM_MAX_SCOPE_ITEM_CHARACTERS,
  DEV_WORKSTREAM_MODES,
  DEV_WORKSTREAM_MUTABLE_STATES,
  DEV_WORKSTREAM_PURPOSES,
  DEV_WORKSTREAM_TERMINAL_STATES,
  DEV_WORKSTREAM_STATES,
  ...
  dev_workspace_lock_owner_status,
...
import {
  ...
  DEV_RETIREMENT_APPROVAL_ID_PATTERN_SOURCE,
  DEV_RETIREMENT_APPROVAL_STATUSES,
  DEV_RETIREMENT_MAX_CONFIRM_BATCH,
  DEV_RETIREMENT_MAX_LIST_RESULTS,
...
const toolDefinitions = [
  ...
  },
  {
    ...
    description: "Create one persistent server-owned development workstream. The server captures current local HEAD as immutable base_head and starts in shared mode; isolated workspace transition is performed only by dev_workspace_create_isolated.",
    risk: "low-risk-write",
    annotations: { readOnlyHint: false },
    inputSchema: baseSchema({
      label: {
        type: "string",
        minLength: 1,
        maxLength: DEV_WORKSTREAM_MAX_LABEL_CHARACTERS,
      },
      purpose: { type: "string", enum: DEV_WORKSTREAM_PURPOSES, default: "primary" },
      ...
      depends_on: {
        type: "array",
        maxItems: DEV_WORKSTREAM_MAX_DEPENDENCIES,
        items: {
          type: "string",
          pattern: DEV_WORKSTREAM_ID_PATTERN_SOURCE,
          maxLength: 64,
      ...
      declared_scope: {
        type: "array",
        maxItems: DEV_WORKSTREAM_MAX_SCOPE_ENTRIES,
        items: { type: "string", maxLength: DEV_WORKSTREAM_MAX_SCOPE_ITEM_CHARACTERS },
      },
      metadata: {
        type: "object",
        maxProperties: DEV_WORKSTREAM_MAX_METADATA_PROPERTIES,
        additionalProperties: {
          oneOf: [
            { type: "string", maxLength: DEV_WORKSTREAM_MAX_METADATA_STRING_CHARACTERS },
            { type: "number" },
            { type: "boolean" },
            { type: "null" },
          ],
  ...
  },
  {
    ...
    description: "Read one persistent development workstream by its opaque server-issued ID, including bounded metadata, revision, dependencies, declared scope, workspace identity, and overlap observation.",
    risk: "read",
    annotations: { readOnlyHint: true },
    inputSchema: baseSchema({
      workstream_id: {
    ...
    handler: async (args) => jsonContent(await dev_workspace_get_workstream(args)),
  },
  {
    name: "dev_workspace_list_workstreams",
    description: "Read a bounded persistent workstream list with safe state, purpose, and active/terminal lifecycle filters. No caller-controlled query language or storage path is accepted.",
    risk: "read",
    annotations: { readOnlyHint: true },
    inputSchema: baseSchema({
      state: { type: "string", enum: DEV_WORKSTREAM_STATES },
      purpose: { type: "string", enum: DEV_WORKSTREAM_PURPOSES },
      lifecycle: { type: "string", enum: DEV_WORKSTREAM_LIST_LIFECYCLES, default: "all" },
      limit: { type: "integer", minimum: 1, maximum: DEV_WORKSTREAM_MAX_LIST_RESULTS, default: DEV_WORKSTREAM_MAX_LIST_RESULTS },
    }),
    handler: async (args) => jsonContent(await dev_workspace_list_workstreams(args)),
  },
  {
    name: "dev_workspace_update_workstream",
    description: "Update bounded mutable metadata or perform a legal non-terminal workstream state transition. Supports an optional expected_revision CAS guard and never changes workstream_id, base_head, created_at, workspace path, or storage path.",
    risk: "low-risk-write",
    annotations: { readOnlyHint: false },
    inputSchema: baseSchema({
      workstream_id: {
      ...
      label: { type: "string", minLength: 1, maxLength: DEV_WORKSTREAM_MAX_LABEL_CHARACTERS },
      purpose: { type: "string", enum: DEV_WORKSTREAM_PURPOSES },
      state: { type: "string", enum: DEV_WORKSTREAM_MUTABLE_STATES },
      parent_workstream_id: {
        type: "string",
        pattern: DEV_WORKSTREAM_ID_PATTERN_SOURCE,
        maxLength: 64,
      ...
      depends_on: {
        type: "array",
        maxItems: DEV_WORKSTREAM_MAX_DEPENDENCIES,
        items: { type: "string", pattern: DEV_WORKSTREAM_ID_PATTERN_SOURCE, maxLength: 64 },
      },
      declared_scope: {
        type: "array",
        maxItems: DEV_WORKSTREAM_MAX_SCOPE_ENTRIES,
    ...
    handler: async (args) => jsonContent(await dev_workspace_update_workstream(args)),
  },
  {
    name: "dev_workspace_end_workstream",
    description: "Formally end a non-terminal persistent workstream as completed or abandoned, with an optional expected_revision CAS guard. History is retained and terminal records cannot be restarted by normal update.",
    risk: "low-risk-write",
    annotations: { readOnlyHint: false },
    inputSchema: baseSchema({
      workstream_id: {
      ...
      outcome: { type: "string", enum: DEV_WORKSTREAM_TERMINAL_STATES },
      expected_revision: { type: "integer", minimum: 1 },
    }, ["workstream_id", "outcome"]),
    handler: async (args) => jsonContent(await dev_workspace_end_workstream(args)),
  },
  {
    name: "dev_workspace_status",
    description: "Read aggregate persistent workstream registry health, counts, current local repository HEAD, active summaries, and base-head divergence observations. Local origin/main tracking is never treated as authoritative remote truth.",
    risk: "read",
    annotations: { readOnlyHint: true },
    inputSchema: baseSchema({}),
  ...
  {
    name: "dev_external_brain_scan_retirement_approvals",
    description: "Run the server-owned external-brain liveness scan and create only deduplicated retirement approval items for stale sessions with complete lineage and no live governance or acceptance-evidence pin. This never retires or deletes a session.",
    risk: "low-risk-write",
    annotations: { readOnlyHint: false },
    inputSchema: baseSchema({
      max_items: { type: "integer", minimum: 1, maximum: DEV_RETIREMENT_MAX_LIST_RESULTS, default: 50 },
    }),
    handler: async (args) => jsonContent(await dev_external_brain_scan_retirement_approvals(args)),
  },
  {
    name: "dev_external_brain_list_retirement_approvals",
    description: "Read a bounded page of external-brain retirement approval items only. Generic approval actions are not exposed through this tool.",
    risk: "read",
    annotations: { readOnlyHint: true },
    inputSchema: baseSchema({
      status: { type: "string", enum: DEV_RETIREMENT_APPROVAL_STATUSES },
      offset: { type: "integer", minimum: 0, maximum: 100000, default: 0 },
      limit: { type: "integer", minimum: 1, maximum: DEV_RETIREMENT_MAX_LIST_RESULTS, default: 50 },
    }),
    handler: async (args) => jsonContent(await dev_external_brain_list_retirement_approvals(args)),
  },
  {
    name: "dev_external_brain_confirm_retirement_approvals",
    description: "High-risk confirmation for an exact bounded set of server-issued external-brain retirement approval IDs. Every ID is preflighted as an actionable retirement request; each session is rechecked for live references before being marked ABANDONED. No cleanup or permanent deletion occurs.",
    risk: "high-risk-write",
    annotations: { readOnlyHint: false },
    inputSchema: baseSchema({
      approval_item_ids: {
        type: "array",
        items: { type: "string", pattern: DEV_RETIREMENT_APPROVAL_ID_PATTERN_SOURCE, maxLength: 64 },
        minItems: 1,
        maxItems: DEV_RETIREMENT_MAX_CONFIRM_BATCH,
        uniqueItems: true,
      },
      confirm: { type: "boolean" },
    }, ["approval_item_ids", "confirm"]),
    handler: async (args) => jsonContent(await dev_external_brain_confirm_retirement_approvals(args)),
  },
  {
  ...
  },
  {
    ...
    description: "Create one server-owned isolated Git worktree for an existing non-terminal shared workstream. Path, branch, Git argv, cwd, environment, base ref, and lock reason are all server-controlled.",
    risk: "low-risk-write",
    annotations: { readOnlyHint: false },
    inputSchema: baseSchema({
      workstream_id: { type: "string", pattern: DEV_WORKSTREAM_ID_PATTERN_SOURCE, maxLength: 64 },
    ...
    handler: async (args) => jsonContent(await dev_workspace_create_isolated(args)),
  },
  {
    name: "dev_workspace_get_workspace",
    description: "Read one server-issued isolated workspace, including persistent mapping and bounded live Git worktree health.",
    risk: "read",
    annotations: { readOnlyHint: true },
    inputSchema: baseSchema({
      workspace_id: { type: "string", pattern: DEV_WORKSPACE_ID_PATTERN_SOURCE, maxLength: 64 },
    }, ["workspace_id"]),
    handler: async (args) => jsonContent(await dev_workspace_get_workspace(args)),
  },
  {
    name: "dev_workspace_list_workspaces",
    description: "Read a bounded list of registered isolated workspaces using only opaque IDs and finite state/type filters.",
    risk: "read",
    annotations: { readOnlyHint: true },
    inputSchema: baseSchema({
      workstream_id: { type: "string", pattern: DEV_WORKSTREAM_ID_PATTERN_SOURCE, maxLength: 64 },
      state: { type: "string", enum: DEV_WORKSPACE_STATES },
      type: { type: "string", enum: DEV_WORKSPACE_TYPES },
      limit: { type: "integer", minimum: 1, maximum: DEV_WORKSPACE_MAX_LIST_RESULTS, default: DEV_WORKSPACE_MAX_LIST_RESULTS },
    }),
    handler: async (args) => jsonContent(await dev_workspace_list_workspaces(args)),
  },
  {
    name: "dev_workspace_lock_owner_status",
    description: "Read-only Windows workspace lock-owner diagnostics for one server-issued isolated workspace. The server derives the exact workspace path; callers cannot supply arbitrary paths or terminate processes.",
    risk: "read",
    annotations: { readOnlyHint: true },
    inputSchema: baseSchema({
      workspace_id: { type: "string", pattern: DEV_WORKSPACE_ID_PATTERN_SOURCE, maxLength: 64 },
    }, ["workspace_id"]),
    handler: async (args) => jsonContent(await dev_workspace_lock_owner_status(args)),
  },
  {
    name: "dev_workspace_lock",
    description: "Apply the server-generated Git worktree lock to one registered active isolated workspace. Caller cannot supply a path, branch, reason, or Git options.",
    risk: "low-risk-write",
    annotations: { readOnlyHint: false },
    inputSchema: baseSchema({
      workspace_id: { type: "string", pattern: DEV_WORKSPACE_ID_PATTERN_SOURCE, maxLength: 64 },
    ...
    handler: async (args) => jsonContent(await dev_workspace_lock(args)),
  },
  {
    name: "dev_workspace_unlock",
    description: "Unlock one registered isolated workspace only when its workstream is not active. Caller cannot supply a path, reason, or Git options.",
    risk: "low-risk-write",
    annotations: { readOnlyHint: false },
    inputSchema: baseSchema({
      workspace_id: { type: "string", pattern: DEV_WORKSPACE_ID_PATTERN_SOURCE, maxLength: 64 },
    ...
    handler: async (args) => jsonContent(await dev_workspace_unlock(args)),
  },
  {
    name: "dev_workspace_remove_isolated",
    description: "Remove one terminal workstream's isolated worktree only when it is clean, mapped consistently, and free of staged, conflicted, or untracked changes. No force removal is available.",
    risk: "low-risk-write",
    annotations: { readOnlyHint: false },
    inputSchema: baseSchema({
      workspace_id: { type: "string", pattern: DEV_WORKSPACE_ID_PATTERN_SOURCE, maxLength: 64 },
  ...
  },
  {
    ...
    description: "[low-risk-write] Architecture-primary formal world-simulation entry. Begins a ChatGPT-owned world-simulation cognition session and may initialize exactly one persisted authoritative world-state snapshot at revision 0 through initial_world_state. It never mutates Canon or active_engine; after bootstrap, hard state evolves only through the native programmatic world loop.",
    risk: "low-risk-write",
    inputSchema: baseSchema({
      simulation_label: { type: "string", maxLength: 240 },
      seed: { type: ["string", "number", "integer"] },
      rules: { type: "object" },
      initial_world_state: {
        type: "object",
        description: "Optional one-time authoritative world-state bootstrap snapshot. Reinitialization of the same world session is rejected.",
      },
      initial_world_state_summary: { type: "object" },
      metadata: { type: "object" },
    }),
    handler: async (args) => jsonContent(await chatgpt_bridge_begin_world_simulation_session(args)),
  },
  {
    name: "chatgpt_bridge_prepare_world_turn",
    description: "[low-risk-write] Architecture-primary formal world-turn preparation. Prepares only the persisted event-queue head through the native loop, keeps the complete PreparedTurn in the parent-process ephemeral broker, and returns exactly one bounded character decision surface plus opaque handles. Callers cannot choose event identity, submit world state, or decide outcomes.",
    risk: "low-risk-write",
    inputSchema: baseSchema({
      world_simulation_session_id: { type: "string" },
  ...
  },
  {
    ...
    }, ["prepared_turn_handle", "decision_handle"], [{
      type: "exactlyOne",
      fields: ["action_id", "reject_all"],
      message: "Submit exactly one character decision: action_id or reject_all=true.",
    }]),
    handler: async (args) => jsonContent(await chatgpt_bridge_submit_world_character_action(args)),
  },
  {
    name: "chatgpt_bridge_resolve_world_turn",
    description: "[low-risk-write] Resolve one fully decided prepared world turn exactly once. The server reacquires the complete PreparedTurn from the parent ephemeral broker, rechecks revision/hash freshness, uses the built-in programmatic causal adjudicator and consistency commit gate, persists memory/history, and commits through the authoritative state writer. Caller-supplied outcomes or next state are impossible by schema.",
    risk: "low-risk-write",
    inputSchema: baseSchema({
      prepared_turn_handle: {
        type: "string",
    ...
    handler: async (args) => jsonContent(await chatgpt_bridge_resolve_world_turn(args)),
  },
  {
    name: "chatgpt_bridge_use_world_scene_causal_analyzer",
    description: "[low-risk-write] Full/debug compatibility capability after Step4B-2. Normalize one Scene State into causal geometry and simultaneous-action adjudication inputs. It prepares evidence only and never decides success, hit, injury, destruction, or victory.",
    risk: "low-risk-write",
    ...
    handler: async (args) => jsonContent(await chatgpt_bridge_use_world_scene_causal_analyzer(args)),
  },
  {
    name: "chatgpt_bridge_use_world_perception_filter",
    description: "[low-risk-write] Full/debug compatibility capability after Step4B-2. Build one character's bounded perception packet from explicit sensory observations and observer-scoped Scene State. Hidden fields, private device contents, and other characters' internal states are excluded.",
    risk: "low-risk-write",
    inputSchema: baseSchema({ world_simulation_session_id: { type: "string" }, capability_input: { type: "object" } }, ["world_simulation_session_id", "capability_input"]),
    handler: async (args) => jsonContent(await chatgpt_bridge_use_world_perception_filter(args)),
  },
  {
  ...
  {
    name: "chatgpt_bridge_use_world_character_cognition",
    description: "[low-risk-write] Full/debug compatibility capability after Step4B-2. Assemble one named character's bounded cognition from perception, memory, needs, emotion, values, relationships, attention, goals, and current action. It cannot choose a world result.",
    risk: "low-risk-write",
    inputSchema: baseSchema({ world_simulation_session_id: { type: "string" }, capability_input: { type: "object" } }, ["world_simulation_session_id", "capability_input"]),
    handler: async (args) => jsonContent(await chatgpt_bridge_use_world_character_cognition(args)),
  },
  {
    name: "chatgpt_bridge_use_world_action_proposer",
    description: "[low-risk-write] Full/debug compatibility capability after Step4B-2. Return non-binding character action intents from simulator-supplied available actions. ChatGPT's character brain chooses the intent; the causal simulator decides the result.",
    risk: "low-risk-write",
    inputSchema: baseSchema({ world_simulation_session_id: { type: "string" }, capability_input: { type: "object" } }, ["world_simulation_session_id", "capability_input"]),
    handler: async (args) => jsonContent(await chatgpt_bridge_use_world_action_proposer(args)),
  },
  {
    name: "chatgpt_bridge_use_world_agency_guard",
    description: "[low-risk-write] Full/debug compatibility capability after Step4B-2. Detect narrative, camera, spotlight, romance, or governance control signals that would improperly choose a character's action or make observation create causality.",
    risk: "low-risk-write",
    inputSchema: baseSchema({ world_simulation_session_id: { type: "string" }, capability_input: { type: "object" } }, ["world_simulation_session_id", "capability_input"]),
    handler: async (args) => jsonContent(await chatgpt_bridge_use_world_agency_guard(args)),
  },
  {
    name: "chatgpt_bridge_use_world_consistency_critic",
    description: "[low-risk-write] Full/debug compatibility capability after Step4B-2. Diagnose explicit world-state transitions for missing causes, duplicate exclusive ownership, knowledge gain without source, and outcome-first shortcuts. It reports only and never repairs history automatically.",
    risk: "low-risk-write",
    inputSchema: baseSchema({ world_simulation_session_id: { type: "string" }, capability_input: { type: "object" } }, ["world_simulation_session_id", "capability_input"]),
    handler: async (args) => jsonContent(await chatgpt_bridge_use_world_consistency_critic(args)),
  },
  {
...
const permissionSources = {
  ...
  dev_workspace_get_workstream: ["development_workstream_registry"],
  dev_workspace_list_workstreams: ["development_workstream_registry"],
  dev_workspace_update_workstream: [
    "development_workstream_registry",
    "mcp_client_bounded_workstream_metadata",
  ...
  dev_external_brain_scan_retirement_approvals: ["external_brain_session_lineage", "external_brain_session_liveness", "approval_queue", "development_operation_journal", "server_owned_retirement_policy"],
  dev_external_brain_list_retirement_approvals: ["approval_queue", "external_brain_session_retirement_requests"],
  dev_external_brain_confirm_retirement_approvals: ["approval_queue", "external_brain_session_lineage", "external_brain_session_liveness", "agent_run_records", "retirement_audit_log", "development_operation_journal", "mcp_client_explicit_confirmation"],
  ...
  dev_workspace_lock_owner_status: ["development_workstream_registry", "server_owned_workspace_path", "windows_system_handle_table"],
  ...
  chatgpt_bridge_use_world_perception_filter: ["user_input", "agent_run_records", "neural_trace_records"],
  chatgpt_bridge_use_world_memory_retriever: ["user_input", "agent_run_records", "neural_trace_records"],
  chatgpt_bridge_use_world_character_cognition: ["user_input", "agent_run_records", "neural_trace_records"],
  chatgpt_bridge_use_world_action_proposer: ["user_input", "agent_run_records", "neural_trace_records"],
  chatgpt_bridge_use_world_agency_guard: ["user_input", "agent_run_records", "neural_trace_records"],
  chatgpt_bridge_use_world_consistency_critic: ["user_input", "agent_run_records", "neural_trace_records"],
