...
async function main() {
  const manifests = [], audits = [];
  for (let index = 2; index < process.argv.length; index += 2) {
    const flag = process.argv[index];
    const filePath = process.argv[index + 1];
    if (!["--manifest", "--audit"].includes(flag) || !filePath) {
      throw new Error("Usage: node tests/run-verification-telemetry.mjs [--manifest file.json] [--audit file.json]...");
    }
  ...
  console.log(JSON.stringify(aggregateVerificationTelemetry({ manifests, audits }), null, 2));
}
main().catch((error) => {
