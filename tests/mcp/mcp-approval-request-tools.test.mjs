...
async function main() {
  ...
  try {
    ...
    // request activation
    const res1 = await approvalRequestTools.request_engine_candidate_activation({ candidate_id: candidateId, reason: 'test' }, { approvalQueue: approvalRoot, pendingEngineCandidates: pendingRoot });
    ...
    const res2 = await approvalRequestTools.request_engine_candidate_activation({ candidate_id: candidateId }, { approvalQueue: approvalRoot, pendingEngineCandidates: pendingRoot });
    ...
    const detail = await approvalRequestTools.get_approval_item_detail({ approval_id: id1 }, { approvalQueue: approvalRoot });
