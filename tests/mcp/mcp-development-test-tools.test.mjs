...
try {
  ...
  assert.equal(typeof persistedFailure.snapshot_fingerprint_cache_eligible, "boolean");
  assert(["none", "memory", "persistent"].includes(persistedFailure.snapshot_fingerprint_cache_source));
  assert.equal(typeof persistedFailure.snapshot_fingerprint_cache_persistent_loaded, "boolean");
  assert(Number.isFinite(persistedFailure.snapshot_fingerprint_cache_source_entry_count));
  assert(Number.isFinite(persistedFailure.snapshot_fingerprint_cache_hit_count));
  assert(Number.isFinite(persistedFailure.snapshot_fingerprint_cache_miss_count));
  assert(Number.isFinite(persistedFailure.snapshot_fingerprint_cache_racy_miss_count));
  assert(Number.isFinite(persistedFailure.snapshot_fingerprint_cache_reused_bytes));
  assert(Number.isFinite(persistedFailure.snapshot_fingerprint_versioned_artifact_count));
  assert(Number.isFinite(persistedFailure.snapshot_fingerprint_version_recheck_ms));
  assert(Number.isFinite(persistedFailure.snapshot_fingerprint_version_recheck_count));
  assert.equal(typeof persistedFailure.snapshot_fingerprint_cache_published, "boolean");
  assert.equal(typeof persistedFailure.snapshot_fingerprint_cache_persistent_published, "boolean");
  assert.equal(typeof persistedFailure.snapshot_authority_available, "boolean");
  assert.equal(typeof persistedFailure.snapshot_authority_query_attempted, "boolean");
  ...
  assert.equal(typeof persistedConcurrent.snapshot_fingerprint_cache_eligible, "boolean");
  assert(["none", "memory", "persistent"].includes(persistedConcurrent.snapshot_fingerprint_cache_source));
  assert.equal(typeof persistedConcurrent.snapshot_fingerprint_cache_persistent_loaded, "boolean");
  assert(Number.isFinite(persistedConcurrent.snapshot_fingerprint_cache_source_entry_count));
  assert(Number.isFinite(persistedConcurrent.snapshot_fingerprint_cache_hit_count));
  assert(Number.isFinite(persistedConcurrent.snapshot_fingerprint_cache_miss_count));
  assert(Number.isFinite(persistedConcurrent.snapshot_fingerprint_cache_racy_miss_count));
  assert(Number.isFinite(persistedConcurrent.snapshot_fingerprint_cache_reused_bytes));
  assert(Number.isFinite(persistedConcurrent.snapshot_fingerprint_versioned_artifact_count));
  assert(Number.isFinite(persistedConcurrent.snapshot_fingerprint_version_recheck_ms));
  assert(Number.isFinite(persistedConcurrent.snapshot_fingerprint_version_recheck_count));
  assert.equal(typeof persistedConcurrent.snapshot_fingerprint_cache_published, "boolean");
  assert.equal(typeof persistedConcurrent.snapshot_fingerprint_cache_persistent_published, "boolean");
  assert.equal(typeof persistedConcurrent.snapshot_authority_available, "boolean");
  assert.equal(typeof persistedConcurrent.snapshot_authority_query_attempted, "boolean");
