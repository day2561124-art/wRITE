...
import {
  ...
  buildWorldSimulationCounterfactualLinkedExperienceLongitudinalReuseOutcomeVariationContract,
  worldSimulationCounterfactualLinkedExperienceLongitudinalReuseOutcomeVariationVersion,
} from "../../server/src/world-simulation-counterfactual-linked-experience-longitudinal-reuse-outcome-variation-service.mjs";
...
const sessionId = "session_phase82b";
const character = "千夜";
const retainedTurnId = "turn_phase82b_retained";
const retainedRevision = 101;
const retainedWorldStateHash = "world_state_hash_phase82b_retained";
...
function phase81MRetention() {
  ...
  const identity = {
    ...
    character,
    source_phase81l_evidence_ref: "phase81l_evidence_phase82b",
    source_phase81l_evidence_hash: "phase81l_evidence_hash_phase82b",
    source_phase81j_projection_hash: "phase81j_projection_hash_phase82b",
    source_phase81i_projection_hash: "phase81i_projection_hash_phase82b",
    reuse_intent_ref: "phase81j_reuse_phase82b",
    reuse_intent_hash: "phase81j_reuse_hash_phase82b",
    source_reentry_candidate_ref: "phase81i_reentry_phase82b",
    source_reentry_candidate_hash: "phase81i_reentry_hash_phase82b",
  ...
  const projection = {
    ...
    state_revision: retainedRevision,
    world_state_hash: retainedWorldStateHash,
    source_phase81l_projection_hash: "phase81l_projection_hash_phase82b",
    source_phase81j_projection_hashes: ["phase81j_projection_hash_phase82b"],
    source_phase81i_projection_hashes: ["phase81i_projection_hash_phase82b"],
    capsule_count: 1,
    capsules: [capsule],
    audit: {
...
function buildReuseTurn(history, { turnId, revision, worldStateHash, result, status, includeStatus = true }) {
  ...
  const phase74A = buildWorldSimulationSubjectiveActionDeliberationView({
    character, cognition, candidate_action_intents: candidates,
  });
...
function buildPhase82A(history, turnId, revision, worldStateHash, current) {
...
function buildPhase82B(history, turnId, revision, worldStateHash, current, phase82a) {
...
const contract = buildWorldSimulationCounterfactualLinkedExperienceLongitudinalReuseOutcomeVariationContract();
assert.equal(contract.phase, "Phase82B");
assert.equal(contract.source_owner, "Phase82A");
assert.equal(contract.per_prior_outcome_comparison_preserved, true);
assert.equal(contract.exact_match_is_not_effectiveness_claim, true);
assert.equal(contract.variation_is_not_failure_claim, true);
...
assert.equal(contract.qualitative_appraisal_deferred_to_separate_phase, true);

const history1 = baseHistory();
const firstTurnId = "turn_phase82b_reuse_1";
const firstRevision = 107;
const firstWorldStateHash = "world_state_hash_phase82b_reuse_1";
...
const secondTurnId = "turn_phase82b_reuse_2";
const secondRevision = 113;
const secondWorldStateHash = "world_state_hash_phase82b_reuse_2";
...
const phase82aExact = buildPhase82A(history2, secondTurnId, secondRevision, secondWorldStateHash, second);
const exactVariation = buildPhase82B(
  history2, secondTurnId, secondRevision, secondWorldStateHash, second, phase82aExact,
);
assert.equal(exactVariation.version, worldSimulationCounterfactualLinkedExperienceLongitudinalReuseOutcomeVariationVersion);
assert.equal(exactVariation.phase, "Phase82B");
assert.equal(exactVariation.comparison_context_count, 1);
const exactContext = exactVariation.comparison_contexts[0];
assert.equal(exactContext.prior_comparison_count, 1);
assert.equal(exactContext.all_prior_subjective_outcomes_exactly_match_current, true);
assert.equal(exactContext.any_subjective_outcome_variation_observed, false);
assert.equal(exactContext.prior_comparisons[0].exact_subjective_outcome_match, true);
assert.equal(exactContext.prior_comparisons[0].subjective_outcome_variation_observed, false);
assert.equal(exactContext.effectiveness_interpretation_performed, false);
assert.equal(exactContext.success_failure_interpretation_performed, false);
assert.equal(exactContext.causal_or_outcome_credit_assigned, false);
assert.doesNotThrow(() =>
  assertWorldSimulationCounterfactualLinkedExperienceLongitudinalReuseOutcomeVariation(
    exactVariation,
    {
      ...
      longitudinal_reuse_outcome_evidence: phase82aExact,
    },
  ));

const thirdHistory = clone(history2);
thirdHistory.turns.push(committedTurn(secondTurnId, secondRevision, secondWorldStateHash, second));
const thirdTurnId = "turn_phase82b_reuse_3";
const thirdRevision = 119;
const thirdWorldStateHash = "world_state_hash_phase82b_reuse_3";
const third = buildReuseTurn(thirdHistory, {
  turnId: thirdTurnId,
  revision: thirdRevision,
  worldStateHash: thirdWorldStateHash,
  result: "paused_before_cover",
  status: "uncertain",
});
const phase82aDifferent = buildPhase82A(thirdHistory, thirdTurnId, thirdRevision, thirdWorldStateHash, third);
const differentVariation = buildPhase82B(
  thirdHistory, thirdTurnId, thirdRevision, thirdWorldStateHash, third, phase82aDifferent,
);
assert.equal(differentVariation.comparison_context_count, 1);
const differentContext = differentVariation.comparison_contexts[0];
assert.equal(differentContext.prior_comparison_count, 2);
assert.equal(differentContext.all_prior_subjective_outcomes_exactly_match_current, false);
assert.equal(differentContext.any_subjective_outcome_variation_observed, true);
assert.equal(differentContext.subjective_outcome_variation_count, 2);
assert.ok(differentContext.prior_comparisons.every((item) => item.subjective_outcome_variation_observed));
assert.ok(differentContext.prior_comparisons.every((item) => item.effectiveness_interpretation_performed === false));

// A Phase82A projection that is locally modified and fully rehashed must still
// fail because Phase82B reconstructs Phase82A from canonical Phase81Q/history sources.
const forged82A = clone(phase82aDifferent);
forged82A.character_case_contexts[0].current_anchor.selected_action_subjective_experience.perceived_result =
  "forged_better_result";
const forgedContext = forged82A.character_case_contexts[0];
const forgedContextIdentity = clone(forgedContext);
delete forgedContextIdentity.context_ref;
delete forgedContextIdentity.context_hash;
delete forgedContextIdentity.effectiveness_interpretation_performed;
delete forgedContextIdentity.success_failure_interpretation_performed;
delete forgedContextIdentity.causal_or_outcome_credit_assigned;
delete forgedContextIdentity.recurrence_count_auto_promotes_rule;
forgedContext.context_hash = hashAgentRunValue(forgedContextIdentity);
forgedContext.context_ref = `phase82a_longitudinal_${forgedContext.context_hash.slice(0, 24)}`;
delete forged82A.projection_hash;
forged82A.projection_hash = hashAgentRunValue(forged82A);
assert.throws(
  () => buildPhase82B(
    thirdHistory, thirdTurnId, thirdRevision, thirdWorldStateHash, third, forged82A,
  ),
  (error) => error?.code
...
const phase82BIndex = loopSource.indexOf(
  "const counterfactualLinkedExperienceLongitudinalReuseOutcomeVariation =",
);
assert.ok(phase82AIndex >= 0);
assert.ok(phase82BIndex > phase82AIndex);
assert.match(
  loopSource,
  /counterfactual_linked_experience_longitudinal_reuse_outcome_variation:\s*\r?\n\s*cloneJson\(counterfactualLinkedExperienceLongitudinalReuseOutcomeVariation\)/,
);
assert.match(
  stateSource,
  /counterfactual_linked_experience_longitudinal_reuse_outcome_variation:\s*\r?\n\s*input\.counterfactual_linked_experience_longitudinal_reuse_outcome_variation \?\? null/,
);
...
console.log(JSON.stringify({
  ...
  exact_canonical_phase82a_reconstruction_required: true,
  per_prior_outcome_structural_comparison_preserved: true,
  exact_match_is_not_effectiveness_claim: true,
  variation_is_not_failure_claim: true,
  rehashed_phase82a_forgery_rejected: true,
  effectiveness_interpretation_performed: false,
  success_failure_interpretation_performed: false,
  reward_or_q_value_modeled: false,
  causal_or_outcome_credit_assigned: false,
  qualitative_appraisal_deferred: true,
  successful_commit_persistence_wired: true,
}));
console.log("Phase82B longitudinal reuse-outcome variation evidence tests passed.");
