...
function createPhase33GOperatorChecklist() {
...
test('Phase33G operator checklist is anchored to Phase33A through Phase33F artifacts', () => {
  ...
  const checklist = createPhase33GOperatorChecklist();

  assert.equal(checklist.phase, '33G');
  assert.equal(checklist.source_phase, '33F');
  assert.equal(checklist.mode, 'operator-checklist');

  assert.deepEqual(checklist.checklist_scope.checks_phase_chain, ['33A', '33B', '33C', '33D', '33E', '33F']);
  assert.equal(checklist.final_closure_under_checklist.phase33a_digest, PHASE_33A_DETERMINISTIC_DIGEST);
  assert.equal(checklist.final_closure_under_checklist.phase33b_digest, PHASE_33B_BRIDGE_PREVIEW_DIGEST);
  assert.equal(checklist.final_closure_under_checklist.phase33c_digest, PHASE_33C_FINAL_SMOKE_DIGEST);
  assert.equal(checklist.final_closure_under_checklist.phase33d_digest, PHASE_33D_ACCEPTANCE_READINESS_DIGEST);
  assert.equal(checklist.final_closure_under_checklist.phase33e_digest, PHASE_33E_ACCEPTANCE_READINESS_FINAL_SMOKE_DIGEST);
  assert.match(checklist.final_closure_under_checklist.source_digest, /^sha256:[0-9a-f]{64}$/);
  assert.equal(checklist.final_closure_under_checklist.boundary, 'final-closure-index');
  assert.equal(checklist.final_closure_under_checklist.closure_kind, 'final');
});

test('Phase33G operator checklist remains read-only and blocks approval/adoption/persistence', () => {
  const checklist = createPhase33GOperatorChecklist();

  assert.equal(checklist.checklist_scope.read_only, true);
  assert.equal(checklist.checklist_scope.operator_checklist_only, true);
  assert.equal(checklist.checklist_scope.inspect_only, true);
  assert.equal(checklist.checklist_scope.no_mcp_tool_addition, true);
  assert.equal(checklist.checklist_scope.no_canon_write, true);
  assert.equal(checklist.checklist_scope.no_active_engine_write, true);
  assert.equal(checklist.checklist_scope.no_compressed_rules_write, true);
  assert.equal(checklist.checklist_scope.no_recovery, true);
  assert.equal(checklist.checklist_scope.no_settlement_context_persistence, true);
  assert.equal(checklist.checklist_scope.no_handoff_persistence, true);
  assert.equal(checklist.checklist_scope.no_candidate_persistence, true);
  assert.equal(checklist.checklist_scope.no_operator_decision_persistence, true);
  assert.equal(checklist.checklist_scope.no_approval, true);
  assert.equal(checklist.checklist_scope.no_adoption, true);

  for (const [assertionName, value] of Object.entries(checklist.checklist_assertions)) {
    assert.equal(value, true, `operator checklist assertion must be true: ${assertionName}`);
  }
});

test('Phase33G operator checklist preserves all safety and mutation flags as false', () => {
  const checklist = createPhase33GOperatorChecklist();

  for (const flag of REQUIRED_FALSE_SAFETY_FLAGS) {
    assert.equal(checklist.safety_flags[flag], false, `safety flag must remain false: ${flag}`);
  }

  assert.deepEqual(checklist.mutation_report.mutation_targets, []);
  assert.equal(checklist.mutation_report.active_engine_modified, false);
  assert.equal(checklist.mutation_report.compressed_rules_modified, false);
  assert.equal(checklist.mutation_report.canon_modified, false);
  assert.equal(checklist.mutation_report.settlement_context_persisted, false);
  assert.equal(checklist.mutation_report.handoff_persisted, false);
  assert.equal(checklist.mutation_report.candidate_persisted, false);
  assert.equal(checklist.mutation_report.operator_decision_persisted, false);
  assert.equal(checklist.mutation_report.approval_confirmed, false);
  assert.equal(checklist.mutation_report.adoption_confirmed, false);
  assert.equal(checklist.mutation_report.recovery_executed, false);
  assert.equal(checklist.mutation_report.rollback_executed, false);
});

test('Phase33G operator checklist is deterministic and registered in run-all', () => {
  const first = createPhase33GOperatorChecklist();
  const second = createPhase33GOperatorChecklist();
  ...
  assert.match(
    runAll,
    /phase33g-aesthetic-memory-context-builder-final-closure-index-bridge-acceptance-readiness-final-closure-operator-checklist\.test\.mjs/,
    'Phase33G test must be registered in tests/run-all.mjs',
  );
});
