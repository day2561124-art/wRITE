...
function previewExplicitCandidateWriteBoundary({
  ...
  return {
    ...
    candidate_record_preview: {
      ...
      requires_future_explicit_write_phase: true,
    },
    ...
    next_allowed_phase: "phase41d_or_later_explicit_candidate_persist_boundary",
    state_after: before,
  };
}
...
{
  const result = previewExplicitCandidateWriteBoundary({
    intakeResult: null,
  });

  assert.equal(result.accepted, false);
  assert.equal(result.reason, "candidate_intake_not_accepted");
  assert.equal(result.candidate_write_preview_opened, false);
...

{
  ...
  assert.equal(result.reason, "explicit_candidate_write_preview_boundary_only");
  assert.equal(result.candidate_write_preview_opened, true);
  assert.equal(result.write_mode, "test_only_preview");
  assert.equal(result.would_create_candidate_record, true);
  ...
  assert.equal(result.candidate_record_preview.kind, "candidate_preview");
  assert.equal(result.candidate_record_preview.persisted, false);
  assert.equal(result.candidate_record_preview.id, null);
  assert.equal(result.candidate_record_preview.source_kind, "chat_output");
  assert.equal(
    result.candidate_record_preview.source_route,
    "chatgpt_native_full_neural_writing_handoff"
  );
  assert.equal(
    result.candidate_record_preview.requires_future_explicit_write_phase,
    true
  );
...

{
  const tamperedIntake = {
    ...
    source_route: "manual_candidate_route",
    direct_requests_detected: {},
  };

  const result = previewExplicitCandidateWriteBoundary({
    intakeResult: tamperedIntake,
  });

  assert.equal(result.accepted, false);
  assert.equal(result.reason, "candidate_write_source_is_not_native_chat_output");
  assert.equal(result.candidate_write_preview_opened, false);
  assertNoMutationSnapshot(result.state_after);
}

{
  ...
  assert.equal(result.accepted, false);
  assert.equal(result.reason, "candidate_intake_contract_invalid");
  assert.equal(result.candidate_write_preview_opened, false);
  assertNoMutationSnapshot(result.state_after);
}

console.log(
  "Phase41C explicit candidate write preview boundary smoke tests passed."
);
