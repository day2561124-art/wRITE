...
function previewExplicitCandidatePersistBoundary({
  ...
  return {
    ...
    next_allowed_phase: "phase41e_or_later_explicit_candidate_runtime_write_gate",
    state_after: before,
  };
}
...
{
  const result = previewExplicitCandidatePersistBoundary({
    writePreviewResult: null,
    userRequest: "persist this candidate preview as a candidate record",
  });

  assert.equal(result.accepted, false);
  assert.equal(result.reason, "candidate_write_preview_not_accepted");
  assert.equal(result.candidate_persist_preview_opened, false);
...

{
  ...
  assert.equal(writePreview.accepted, false);
  assert.equal(persistPreview.accepted, false);
  assert.equal(persistPreview.reason, "candidate_write_preview_not_accepted");
  assert.equal(persistPreview.candidate_persist_preview_opened, false);
  assertNoMutationSnapshot(persistPreview.state_after);
}

{
  ...
  assert.equal(persistPreview.accepted, false);
  assert.equal(
    persistPreview.reason,
    "missing_explicit_candidate_persist_request"
  );
  assert.equal(persistPreview.candidate_persist_preview_opened, false);
  assertNoMutationSnapshot(persistPreview.state_after);
}

{
  ...
  assert.equal(
    persistPreview.reason,
    "explicit_candidate_persist_boundary_preview_only"
  );
  assert.equal(persistPreview.candidate_persist_preview_opened, true);
  assert.equal(persistPreview.persist_mode, "test_only_preview");
  assert.equal(persistPreview.would_persist_candidate_record, true);

  assert.equal(persistPreview.candidate_saved, false);
  assert.equal(persistPreview.candidate_persist_performed, false);
  assert.equal(persistPreview.candidate_id_created, false);
  assert.equal(persistPreview.approval_request_created, false);
  assert.equal(persistPreview.pending_engine_candidate_created, false);
  assert.equal(persistPreview.adoption_performed, false);
  assert.equal(persistPreview.settlement_performed, false);
  assert.equal(persistPreview.canon_update_performed, false);
  assert.equal(persistPreview.active_engine_update_performed, false);

  assert.equal(
    persistPreview.candidate_persist_preview.kind,
    "candidate_persist_preview"
  );
  assert.equal(persistPreview.candidate_persist_preview.persisted, false);
  assert.equal(persistPreview.candidate_persist_preview.id, null);
  assert.equal(
    persistPreview.candidate_persist_preview.source_candidate_preview_kind,
    "candidate_preview"
  );
  assert.equal(
    persistPreview.candidate_persist_preview.requires_future_runtime_write_implementation,
    true
  );
...

{
  ...
  assert.equal(persistPreview.accepted, true);
  assert.equal(persistPreview.direct_requests_detected.approval, true);
  assert.equal(persistPreview.direct_requests_detected.pending_engine, true);
  assert.equal(persistPreview.direct_requests_detected.adoption, true);
  assert.equal(persistPreview.direct_requests_detected.settlement, true);
  assert.equal(persistPreview.direct_requests_detected.canon, true);
  assert.equal(persistPreview.direct_requests_detected.active_engine, true);

  assert.ok(
    persistPreview.blocked_direct_actions.includes("candidate_persist")
  );
  assert.ok(
    persistPreview.blocked_direct_actions.includes("approval_request_create")
  );
  assert.ok(
    persistPreview.blocked_direct_actions.includes(
      "pending_engine_candidate_create"
    )
  );
  assert.ok(persistPreview.blocked_direct_actions.includes("adoption"));
  assert.ok(persistPreview.blocked_direct_actions.includes("settlement"));
  assert.ok(persistPreview.blocked_direct_actions.includes("canon_update"));
  assert.ok(
    persistPreview.blocked_direct_actions.includes("active_engine_update")
  );
...

{
  ...
  const result = previewExplicitCandidatePersistBoundary({
    writePreviewResult: tamperedWritePreview,
    userRequest: "persist this candidate preview as a candidate record",
  });

  assert.equal(result.accepted, false);
  assert.equal(result.reason, "candidate_write_preview_contract_invalid");
  assert.equal(result.candidate_persist_preview_opened, false);
  assertNoMutationSnapshot(result.state_after);
}
...
console.log(
  "Phase41D explicit candidate persist boundary smoke tests passed."
);
