...
const CURRENT_PHASE = "Phase42S";
const CURRENT_PHASE_SLUG = "phase42s-production-candidate-store-promotion-gate-backup-snapshot-creation-denial-smoke";
const PREVIOUS_PHASE = "Phase42R";
const PREVIOUS_PHASE_SLUG = "phase42r-production-candidate-store-promotion-gate-audit-receipt-creation-denial-smoke";
const SOURCE_PHASE = "Phase42Q";
const SOURCE_PHASE_SLUG = "phase42q-production-candidate-store-promotion-gate-production-gate-open-execution-denial-smoke";
...
const DENIAL_TRUE_FIELDS = [
  "phase42s_backup_snapshot_creation_denial_generated",
  "phase42r_audit_receipt_creation_denial_accepted",
  "explicit_backup_snapshot_creation_request_detected",
  "backup_snapshot_creation_denied",
  "backup_snapshot_creation_not_authorized",
...
function makePhase42RAuditReceiptCandidate(overrides = {}) {
  ...
  return Object.freeze({
    ...
    source_phase_slug: SOURCE_PHASE_SLUG,
...
function makePhase42RAuditReceiptCreationDenial(overrides = {}) {
  const sourceState = makeBaselineState(overrides.state_before ?? {});
  const stateAfter = clone(overrides.state_after ?? sourceState);
  const auditReceiptCandidate = makePhase42RAuditReceiptCandidate(overrides.audit_receipt_candidate ?? {});
  ...
  return Object.freeze({
    ...
    explicit_audit_receipt_creation_request_detected: true,
    audit_receipt_creation_denied: true,
    ...
    audit_receipt_scope_hash: stableHash({
...
function makeBackupSnapshotCandidate(sourceDenial, overrides = {}) {
  const sourceContractHash = stableHash({
    phase: sourceDenial.phase,
    phase_slug: sourceDenial.phase_slug,
    reason: sourceDenial.reason,
    mode: sourceDenial.mode,
    audit_receipt_creation_denied: sourceDenial.audit_receipt_creation_denied,
...
function makeBackupSnapshotCreationRequest(overrides = {}) {
  return Object.freeze({
    kind: "production_candidate_store_promotion_gate_backup_snapshot_creation_request",
    explicit: true,
    text: [
      "Create the backup snapshot for the Phase42R audit receipt creation denial.",
      "This is an explicit backup snapshot creation request.",
      "The backup snapshot creation request must be denied and no backup snapshot may be created.",
      "Keep the audit receipt absent.",
      ...
      "Do not execute rollback restore.",
      "Do not claim full run-all passed."
    ].join(" "),
    requested_action: "create_backup_snapshot",
    target_phase: CURRENT_PHASE,
    source_phase: PREVIOUS_PHASE,
    preview_only: true,
    authorization: {
      operator_confirmed: true,
      phase42r_denial_accepted: true,
...
function classifyIntent(text) {
  return Object.freeze({
    backup_snapshot_create: /\b(create|make|persist|capture|write)\b[\s\S]*\bbackup snapshot\b/i.test(text) ||
...
function validatePhase42RAuditReceiptCandidate(candidate) {
  ...
  assert.equal(candidate.source_phase, SOURCE_PHASE);
  assert.equal(candidate.source_phase_slug, SOURCE_PHASE_SLUG);
  assert.equal(candidate.source_denial_reason, "production_gate_open_execution_denied_preview_only");
  assert.equal(candidate.source_denial_mode, "production_gate_open_execution_denial_smoke");
...
function validatePhase42RDenial(sourceDenial) {
  assert.equal(sourceDenial.phase, PREVIOUS_PHASE);
  assert.equal(sourceDenial.phase_slug, PREVIOUS_PHASE_SLUG);
  assert.equal(sourceDenial.source_phase, SOURCE_PHASE);
  assert.equal(sourceDenial.source_phase_slug, SOURCE_PHASE_SLUG);
  assert.equal(sourceDenial.accepted, true);
  assert.equal(sourceDenial.reason, "audit_receipt_creation_denied_preview_only");
  assert.equal(sourceDenial.mode, "audit_receipt_creation_denial_smoke");
  assert.equal(sourceDenial.preview_only, true);
  assert.equal(sourceDenial.persisted, false);
  assert.equal(sourceDenial.phase42r_audit_receipt_creation_denial_generated, true);
  assert.equal(sourceDenial.phase42r_audit_receipt_creation_denial_persisted, false);
  assert.equal(sourceDenial.phase42q_production_gate_open_execution_denial_accepted, true);
  assert.equal(sourceDenial.explicit_audit_receipt_creation_request_detected, true);
  assert.equal(sourceDenial.audit_receipt_creation_denied, true);
  assert.equal(sourceDenial.audit_receipt_creation_not_authorized, true);
  ...
  const expectedAuditReceiptScopeHash = stableHash({
    audit_receipt_id: sourceDenial.audit_receipt_candidate.audit_receipt_id,
    source_phase: sourceDenial.audit_receipt_candidate.source_phase,
    source_phase_slug: sourceDenial.audit_receipt_candidate.source_phase_slug,
    request_scope: sourceDenial.audit_receipt_candidate.request_scope,
    audit_scope: sourceDenial.audit_receipt_candidate.audit_scope,
    receipt_mode: sourceDenial.audit_receipt_candidate.receipt_mode,
    creation_allowed: sourceDenial.audit_receipt_candidate.creation_allowed,
    created: sourceDenial.audit_receipt_candidate.created,
    persisted: sourceDenial.audit_receipt_candidate.persisted
  });
  assert.equal(sourceDenial.audit_receipt_scope_hash, expectedAuditReceiptScopeHash);

  assert.equal(sourceDenial.denial_packet.kind, "production_candidate_store_promotion_gate_audit_receipt_creation_denial_packet");
  assert.equal(sourceDenial.denial_packet.denied_action, "create_audit_receipt");
  assert.equal(sourceDenial.denial_packet.preview_only, true);
  assert.equal(sourceDenial.denial_packet.audit_receipt_creation_denied, true);
  assert.equal(sourceDenial.denial_packet.audit_receipt_creation_authorized, false);
  ...
  assert.deepEqual(sourceDenial.state_after, sourceDenial.state_before, "Phase42R state snapshots must be value-equal");
}

function validateBackupSnapshotCandidate(candidate, sourceDenial) {
...
function validateBackupSnapshotCreationRequest(request) {
  assert.equal(request.kind, "production_candidate_store_promotion_gate_backup_snapshot_creation_request");
  assert.equal(request.explicit, true);
  assert.equal(request.requested_action, "create_backup_snapshot");
  ...
  assert.equal(intent.backup_snapshot_create, true, "explicit backup snapshot creation request must be detectable");
  assert.equal(request.authorization.operator_confirmed, true);
  assert.equal(request.authorization.phase42r_denial_accepted, true);
  assert.equal(request.authorization.backup_snapshot_creation_allowed, false);
...
function buildBackupSnapshotCreationDenial({
  sourceDenial,
  request,
  state = makeBaselineState(),
  backupSnapshotCandidateOverrides = {}
} = {}) {
  validatePhase42RDenial(sourceDenial);
  const intent = validateBackupSnapshotCreationRequest(request);
  ...
  const backupSnapshotCandidate = makeBackupSnapshotCandidate(sourceDenial, backupSnapshotCandidateOverrides);
  validateBackupSnapshotCandidate(backupSnapshotCandidate, sourceDenial);

  const backupSnapshotScopeHash = stableHash({
  ...
  const denial = {
    ...
    explicit_backup_snapshot_creation_request_detected: intent.backup_snapshot_create,
    ...
    backup_snapshot_scope_hash: backupSnapshotScopeHash,
    denial_packet: {
  ...
  for (const field of DENIAL_TRUE_FIELDS) {
    assert.equal(denial[field], true, `${field} must be true in backup snapshot creation denial`);
  }
...
{
  const result = buildBackupSnapshotCreationDenial({
    sourceDenial: makePhase42RAuditReceiptCreationDenial(),
    request: makeBackupSnapshotCreationRequest()
  });

  assert.equal(result.accepted, true);
  assert.equal(result.reason, "backup_snapshot_creation_denied_preview_only");
  assert.equal(result.backup_snapshot_creation_denied, true);
  assert.equal(result.backup_snapshot_creation_not_authorized, true);
...
for (const previousOverride of [
  { accepted: false },
  { phase42r_audit_receipt_creation_denial_generated: false },
  { phase42r_audit_receipt_creation_denial_persisted: true },
  { phase42q_production_gate_open_execution_denial_accepted: false },
  { explicit_audit_receipt_creation_request_detected: false },
  { audit_receipt_creation_denied: false },
  { audit_receipt_creation_not_authorized: false },
  ...
  assertThrowsWithMessage(
    () => buildBackupSnapshotCreationDenial({
      sourceDenial: makePhase42RAuditReceiptCreationDenial(previousOverride),
      request: makeBackupSnapshotCreationRequest()
    }),
...
for (const protectedOverride of [
  ...
  assertThrowsWithMessage(
    () => buildBackupSnapshotCreationDenial({
      sourceDenial: makePhase42RAuditReceiptCreationDenial(protectedOverride),
      request: makeBackupSnapshotCreationRequest()
    }),
    /must remain false/
  );
...
for (const packetOverride of [
  { audit_receipt_creation_authorized: true },
  { verifies_phase42q_gate_open_execution_denial: false },
  ...
  assertThrowsWithMessage(
    () => buildBackupSnapshotCreationDenial({
      sourceDenial: makePhase42RAuditReceiptCreationDenial({ denial_packet: packetOverride }),
      request: makeBackupSnapshotCreationRequest()
    }),
    /Expected values to be strictly equal|must be true|must remain false/
  );
...
for (const sourceCandidateOverride of [
  { preview_only: false },
  ...
  assertThrowsWithMessage(
    () => buildBackupSnapshotCreationDenial({
      sourceDenial: makePhase42RAuditReceiptCreationDenial({ audit_receipt_candidate: sourceCandidateOverride }),
      request: makeBackupSnapshotCreationRequest()
    }),
    /Expected values to be strictly equal|must be true|must remain false|did not match/
  );
}
...
assertThrowsWithMessage(
  () => buildBackupSnapshotCreationDenial({
    sourceDenial: makePhase42RAuditReceiptCreationDenial({
      state_after: { ...makeBaselineState(), backup_snapshot_created: true }
    }),
    request: makeBackupSnapshotCreationRequest()
  }),
  /must remain false/
...
assertThrowsWithMessage(
  () => buildBackupSnapshotCreationDenial({
    sourceDenial: makePhase42RAuditReceiptCreationDenial(),
    request: makeBackupSnapshotCreationRequest({ explicit: false })
  }),
  /Expected values to be strictly equal|must be true|must remain false/
);
...
assertThrowsWithMessage(
  () => buildBackupSnapshotCreationDenial({
    sourceDenial: makePhase42RAuditReceiptCreationDenial(),
    request: makeBackupSnapshotCreationRequest({
      text: "Only inspect the Phase42R denial packet without requesting any backup action."
    })
  }),
  /explicit backup snapshot creation request must be detectable/
);

for (const authorizationOverride of [
  { operator_confirmed: false },
  { phase42r_denial_accepted: false },
  { backup_snapshot_creation_allowed: true },
  { audit_receipt_created: true },
...
for (const candidateOverride of [
  ...
  assertThrowsWithMessage(
    () => buildBackupSnapshotCreationDenial({
      sourceDenial: makePhase42RAuditReceiptCreationDenial(),
      request: makeBackupSnapshotCreationRequest(),
      backupSnapshotCandidateOverrides: candidateOverride
    }),
    /Expected values to be strictly equal|must be true|must remain false/
  );
}
...

{
  ...
  const result = buildBackupSnapshotCreationDenial({
    sourceDenial: makePhase42RAuditReceiptCreationDenial(),
    request: makeBackupSnapshotCreationRequest({ text: riskyText })
  });
...
console.log("Phase42S production candidate store promotion gate backup snapshot creation denial smoke tests passed.");
