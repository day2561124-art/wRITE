...
async function assertSnapshotUnchanged(before) {
  for (const [filePath, prior] of before) {
    try {
      ...
      assert.notEqual(prior, null, "File was created by public profile visual guard MCP tool: " + filePath);
      assert(current.equals(prior), "File changed after public profile visual guard MCP tool: " + filePath);
    } catch (error) {
      if (error.code !== "ENOENT") throw error;
      assert.equal(prior, null, "File was removed by public profile visual guard MCP tool: " + filePath);
    }
  }
}
...
function fakeContextBundle() {
  const visual_uploaded_references = {
    ...
    items: [
      {
        visual_id: "VIS-PHASE39P-MISAKI-REVERSAL",
        character: "御先反轉型態",
      ...
      {
        visual_id: "VIS-PHASE39P-CATWOLF",
        character: "貓狼",
  ...
  return {
    bundle_id: "phase39p-visual-reference-consumer-guard-mcp-public-profile-exposure-smoke-context",
    ...
    proofing_card_excerpt_or_reference: {
      text: "Public profile exposure must list and call the readonly visual reference guard without mutation.",
    },
    longline_excerpt_or_reference: {
      text: "長線承接只依文字正史。",
    },
    retrieval_context: { phase: "39P", visual_uploaded_references },
    generation_context: { phase: "39P", visual_uploaded_references },
  };
}
...
function startPublicMcpServer() {
  ...
  child.on("exit", (code, signal) => {
    for (const [id, entry] of pending) {
      entry.reject(new Error("MCP server exited before response " + id + ": code=" + code + " signal=" + signal + "\n" + stderrBuffer));
    }
    pending.clear();
  });
...
const result = await buildChatgptNativeNeuralWritingHandoff({
  ...
  generation_context: {
    phase: "39P",
    route_expectation: "visual_reference_consumer_guard_mcp_public_profile_exposure_smoke",
  },
  retrieval_context: { acceptance_smoke: "phase39p" },
...
try {
  const init = await server.call("initialize", {
    ...
    capabilities: {},
    clientInfo: { name: "phase39p-public-profile-smoke", version: "0.0.0" },
  });
  ...
  assert.match(tool.description, /^\[read\]/u);
  assert.equal(tool.annotations.readOnlyHint, true);
  assert.equal(tool.inputSchema.properties.outputText.type, "string");
  assert.equal(tool.inputSchema.properties.consumerContract.type, "object");
  assert.equal(tool.inputSchema.properties.maxExcerptChars.type, "integer");
  assert(tool.inputSchema.required.includes("outputText"));
  assert(tool.inputSchema.required.includes("consumerContract"));
  ...
  assert(permission.allowed_sources.includes("visual_reference_consumer_guard"));
  assert(permission.allowed_sources.includes("mcp_readonly_preview"));
  assert(permission.allowed_sources.includes("user_input"));

  for (const forbiddenPublicTool of ["activate_engine_version", "import_policy_file", "commit_error_report", "compress_error_rules"]) {
    assert(!listed.tools.some((item) => item.name === forbiddenPublicTool), forbiddenPublicTool + " must not be listed in chatgpt_public profile.");
  }
  ...
  const validCall = await server.call("tools/call", {
    ...
    arguments: {
      ...
      consumerContract,
      packetId: "phase39p-valid-public-profile-preview",
      maxExcerptChars: 80,
    },
  });
  assert.notEqual(validCall.isError, true, "valid public profile call unexpectedly failed.");
  ...
  const invalidCall = await server.call("tools/call", {
    ...
    arguments: {
      ...
      consumerContract,
      packetId: "phase39p-invalid-public-profile-preview",
      maxExcerptChars: 24,
    },
  });
  assert.notEqual(invalidCall.isError, true, "invalid public profile call should return readonly blocked payload, not transport error.");
  ...
  const disabledCall = await server.call("tools/call", {
    ...
    arguments: {
      outputText: invalidOutput,
      consumerContract: {},
      packetId: "phase39p-disabled-public-profile-preview",
    },
  });
...
assert((await readFile(projectPaths.activeEngine)).equals(activeBefore), "Public profile readonly preview changed active_engine.");
assert(sameSet(await names(path.join(projectPaths.canonDb, "pending_engine_candidates")), pendingBefore), "Public profile readonly preview changed pending candidates.");
assert(sameSet(await names(path.join(projectPaths.approvalQueue, "items")), approvalBefore), "Public profile readonly preview changed approval queue.");
assert(sameSet(await names(path.join(projectPaths.cleanupRoot, "proposals")), cleanupBefore), "Public profile readonly preview changed cleanup proposals.");

for (const forbiddenTopLevel of ["chatgpt_final_output", "extracted_chatgpt_final_output", "final_response_handoff_for_chat", "generation_provider_required"]) {
...
assert.match(serverSource, /readonlyTools\.preview_visual_reference_consumer_output_guard/u);
assert.match(serverSource, /visual_reference_consumer_guard/u);
assert.match(serverSource, /mcp_readonly_preview/u);
assert.match(serverSource, /readOnlyHint: true/u);

console.log("Phase39P visual reference consumer guard MCP public profile exposure smoke tests passed.");
