...
async function assertSnapshotUnchanged(before) {
  for (const [filePath, prior] of before) {
    try {
      ...
      assert.notEqual(prior, null, "File was created by ChatGPT action surface visual guard smoke: " + filePath);
      assert(current.equals(prior), "File changed after ChatGPT action surface visual guard smoke: " + filePath);
    } catch (error) {
      if (error.code !== "ENOENT") throw error;
      assert.equal(prior, null, "File was removed by ChatGPT action surface visual guard smoke: " + filePath);
    }
  }
}
...
function fakeContextBundle() {
  const visual_uploaded_references = {
    ...
    items: [
      {
        visual_id: "VIS-PHASE39Q-MISAKI-REVERSAL",
        character: "御先反轉型態",
      ...
      {
        visual_id: "VIS-PHASE39Q-CATWOLF",
        character: "貓狼",
  ...
  return {
    bundle_id: "phase39q-visual-reference-consumer-guard-chatgpt-action-surface-final-smoke-context",
    ...
    proofing_card_excerpt_or_reference: {
      text: "ChatGPT action surface final smoke must remain readonly and reference-only.",
    },
    longline_excerpt_or_reference: {
      text: "長線承接只依文字正史。",
    },
    retrieval_context: { phase: "39Q", visual_uploaded_references },
    generation_context: { phase: "39Q", visual_uploaded_references },
  };
}
...
const result = await buildChatgptNativeNeuralWritingHandoff({
  ...
  generation_context: {
    phase: "39Q",
    route_expectation: "visual_reference_consumer_guard_chatgpt_action_surface_final_smoke",
  },
  retrieval_context: { acceptance_smoke: "phase39q" },
...
try {
  const init = await server.call("initialize", {
    ...
    capabilities: {},
    clientInfo: { name: "phase39q-action-surface-smoke", version: "0.0.0" },
  });
  ...
  assert.equal(actionSurface.used, true);
  assert.equal(actionSurface.phase, "39Q");
  assert.equal(
    actionSurface.surface_kind,
    "visual_reference_consumer_guard_chatgpt_action_surface_final_smoke",
  );
  assert.equal(actionSurface.action_surface_kind, "chatgpt_action_surface_readonly_guard_preview");
  assert.equal(actionSurface.tool_name, "preview_visual_reference_consumer_output_guard");
  assert.equal(actionSurface.source_tool_profile, "chatgpt_public");
  assert.equal(actionSurface.action_surface_ready, true);
  assert.equal(actionSurface.listed_in_chatgpt_public, true);
  assert.equal(actionSurface.callable_via_tools_call, true);
  assert.equal(actionSurface.exposes_guard_preview, true);
  assert.equal(actionSurface.read_only, true);
  assert.equal(actionSurface.permission_level, "read_only");
  assert.equal(actionSurface.read_or_write, "read");
  assert.equal(actionSurface.requires_user_confirmation, false);
  assert.equal(actionSurface.requires_backup_before_write, false);
  assert.equal(actionSurface.can_modify_canon, false);
  assert.equal(actionSurface.can_modify_active_engine, false);
  assert.equal(actionSurface.can_modify_story_graph, false);
  assert.equal(actionSurface.can_modify_memory, false);
  assert.equal(actionSurface.log_required, false);
  ...
  assert.equal(actionSurface.output_is_reference_only, true);
  assert.equal(actionSurface.action_surface_must_not_replace_final_output, true);
  assert.equal(actionSurface.action_surface_must_not_be_emitted_as_story_text, true);
  assert(actionSurface.allowed_sources.includes("visual_reference_consumer_guard"));
  assert(actionSurface.allowed_sources.includes("mcp_readonly_preview"));
  assert(actionSurface.allowed_sources.includes("user_input"));
  assert.equal(actionSurface.safety_flags.visual_reference_guard_chatgpt_action_surface_final_smoke_ready, true);
  assert.equal(actionSurface.safety_flags.candidate_created, false);
  assert.equal(actionSurface.safety_flags.canon_updated, false);
  assert.equal(actionSurface.safety_flags.active_engine_updated, false);
  assert.equal(actionSurface.safety_flags.adopted, false);
  assert.equal(actionSurface.safety_flags.settled, false);
  ...
  const validCall = await server.call("tools/call", {
    ...
    arguments: {
      ...
      consumerContract,
      packetId: "phase39q-valid-action-surface-preview",
      maxExcerptChars: 80,
    },
  });
  assert.notEqual(validCall.isError, true, "valid action surface call unexpectedly failed.");
  ...
  assert.equal(validData.data.safety_flags.candidate_created, false);
  assert.equal(validData.data.safety_flags.canon_updated, false);
  assert.equal(validData.data.safety_flags.active_engine_updated, false);
  assert.equal(validData.data.safety_flags.adopted, false);
  assert.equal(validData.data.safety_flags.settled, false);
  ...
  const invalidCall = await server.call("tools/call", {
    ...
    arguments: {
      ...
      consumerContract,
      packetId: "phase39q-invalid-action-surface-preview",
      maxExcerptChars: 24,
    },
  });
  ...
  assert.equal(invalidData.data.safety_flags.candidate_created, false);
  assert.equal(invalidData.data.safety_flags.canon_updated, false);
  assert.equal(invalidData.data.safety_flags.active_engine_updated, false);
  assert.equal(invalidData.data.safety_flags.adopted, false);
  assert.equal(invalidData.data.safety_flags.settled, false);
  ...
  const disabledCall = await server.call("tools/call", {
    ...
    arguments: {
      outputText: invalidOutput,
      consumerContract: {},
      packetId: "phase39q-disabled-action-surface-preview",
    },
  });
  ...
  assert.equal(disabledData.data.safety_flags.candidate_created, false);
  assert.equal(disabledData.data.safety_flags.canon_updated, false);
  assert.equal(disabledData.data.safety_flags.active_engine_updated, false);
  assert.equal(disabledData.data.safety_flags.adopted, false);
  assert.equal(disabledData.data.safety_flags.settled, false);
} finally {
  await server.close();
}

await assertSnapshotUnchanged(watched);
assert((await readFile(projectPaths.activeEngine)).equals(activeBefore), "ChatGPT action surface smoke changed active_engine.");
assert(
  sameSet(await names(path.join(projectPaths.canonDb, "pending_engine_candidates")), pendingBefore),
  "ChatGPT action surface smoke changed pending candidates.",
);
assert(
  sameSet(await names(path.join(projectPaths.approvalQueue, "items")), approvalBefore),
  "ChatGPT action surface smoke changed approval queue.",
);
assert(
  sameSet(await names(path.join(projectPaths.cleanupRoot, "proposals")), cleanupBefore),
  "ChatGPT action surface smoke changed cleanup proposals.",
);
...
assert.match(serverSource, /chatgptActionSurfaceMetadata/u);
assert.match(serverSource, /armed-academy\/chatgpt_action_surface/u);
assert.match(serverSource, /visual_reference_consumer_guard_chatgpt_action_surface_final_smoke/u);
assert.match(serverSource, /preview_visual_reference_consumer_output_guard/u);
...
console.log("Phase39Q visual reference consumer guard ChatGPT action surface final smoke tests passed.");
