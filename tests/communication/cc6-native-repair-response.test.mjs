...
const fixtureRoot = path.join(
  ...
  "tests",
  ".tmp",
  `cc6-native-repair-response-${process.pid}-${Date.now()}`,
);
...
try {
  const session = await beginWorldSimulationSession({
    simulation_label: "CC-6I original-speaker repair response",
    seed: "cc6-native-repair-response",
    rules: {
      event_driven: true,
  ...
  assert.equal(
    bPacketAfterResponse.perception.information_boundary.communication_listener_signal_reception_verified,
    true,
  );
  assert.equal(
    bPacketAfterResponse.perception.information_boundary.communication_listener_grounding_claimed,
    false,
  );
  const finalHistory = await getWorldSimulationHistory(session.world_simulation_session_id, options);
  const received = finalHistory.turns.at(-1).communication_listener_understanding_projections.find(
    (item) => item.character === "B");
  assert.equal(received.candidate_count, 1);
  assert.equal(received.decision_count, 0);
  assert.equal(finalHistory.turns.at(-1).communication_repair_response_projections.find(
    (item) => item.character === "B").proposed_count, 0);
} finally {
  await rm(fixtureRoot, { recursive: true, force: true });
}
console.log("CC-6I original-speaker repair response tests passed.");
