...
import { worldSimulationChronologicalMutationQueueVersion } from "../../server/src/world-simulation-chronological-mutation-queue-service.mjs";
import {
...
const readinessContract = buildWorldSimulationObserverTickSnapshotReadinessContract();
assert.equal(readinessContract.snapshot_world_state_materialized, false);
assert.equal(readinessContract.all_queued_mutation_times_must_be_exact, true);
assert.equal(readinessContract.character_brain_invoked_here, false);

const exactBatch = (time, id, precision = "exact") => ({
  time_ms: time,
  chain_hash_after: "chain-" + id,
  mutations: [{ mutation_id: id, time_ms: time, time_precision: precision }],
});
const assembleQueue = (batches, overrides = {}) => {
  const mutations = batches.flatMap((batch) => batch.mutations);
  const queue = {
    version: worldSimulationChronologicalMutationQueueVersion,
    turn_id: "test-turn",
    mutation_count: mutations.length,
    batch_count: batches.length,
    terminal_chain_hash: "terminal-chain",
    inferred_timestamp_mutation_count:
      mutations.filter((item) => item.time_precision !== "exact").length,
...
const exactQueue = assembleQueue([
  exactBatch(stream.increments[0].end_offset_ms, "early"),
  exactBatch(stream.increments.at(-1).end_offset_ms, "late"),
]);
const attested = (queue) => ({
  queue_hash: queue.queue_hash,
  all_preview_changes_reproduced_by_queue: true,
  sole_final_world_state_writer: true,
});
const checked = buildWorldSimulationObserverTickSnapshotReadiness({
  ledger,
  chronological_mutation_queue: exactQueue,
  chronological_mutation_execution: attested(exactQueue),
});
assert.equal(checked.status, "exact_prefix_readiness_only");
assert.equal(checked.ready_tick_count, ledger.tick_count);
assert.equal(checked.ticks[0].complete_same_timestamp_mutation_count, 1);
assert.equal(checked.ticks.at(-1).complete_same_timestamp_mutation_count, 2);
assert.equal(checked.ticks.at(-1).world_snapshot_materialized, false);
const splitSameTimestampQueue = assembleQueue([
  exactBatch(stream.increments[0].end_offset_ms, "first"),
  exactBatch(stream.increments[0].end_offset_ms, "illegal-second"),
]);
assert.throws(() => buildWorldSimulationObserverTickSnapshotReadiness({
  ledger, chronological_mutation_queue: splitSameTimestampQueue,
  chronological_mutation_execution: attested(splitSameTimestampQueue),
}), /ordered bounded mutation batches/u);
assert(!JSON.stringify(checked).includes(semantic));
assert(!JSON.stringify(checked).includes(surface));
assert(!JSON.stringify(checked).includes(actionId));
const inferredQueue = assembleQueue([
  exactBatch(stream.increments[0].end_offset_ms, "early"),
  exactBatch(stream.increments.at(-1).end_offset_ms, "late", "turn_end_inferred"),
]);
const inferred = buildWorldSimulationObserverTickSnapshotReadiness({
  ledger, chronological_mutation_queue: inferredQueue,
  chronological_mutation_execution: attested(inferredQueue),
});
assert.equal(inferred.status, "snapshot_not_safe");
assert.equal(inferred.ready_tick_count, 0);
assert(inferred.ticks.every((item) =>
  item.snapshot_ready === false
  && item.refusal_reason === "mutation_occurrence_time_inferred"
  && item.verified_mutation_prefix_ref === null));
const unattested = buildWorldSimulationObserverTickSnapshotReadiness({
  ledger, chronological_mutation_queue: exactQueue,
});
assert.equal(unattested.ticks[0].refusal_reason,
  "mutation_executor_attestation_unavailable");
const discontinuousQueue = assembleQueue([exactBatch(
  stream.increments[0].end_offset_ms, "early",
)], { continuity_warning_count: 1 });
assert.equal(buildWorldSimulationObserverTickSnapshotReadiness({
  ledger, chronological_mutation_queue: discontinuousQueue,
  chronological_mutation_execution: attested(discontinuousQueue),
}).ticks[0].refusal_reason, "mutation_continuity_unverified");
assert.equal(buildWorldSimulationObserverTickSnapshotReadiness({
  ledger,
}).ticks[0].refusal_reason, "authoritative_mutation_queue_unavailable");
assert.equal(buildWorldSimulationObserverTickSnapshotReadiness({
  ledger: emptyCustomTurn,
}).status, "no_observer_release_ticks");
assert.throws(() => buildWorldSimulationObserverTickSnapshotReadiness({
  ledger: { ...ledger, tick_count: ledger.tick_count + 1 },
}), /release ledger/u);
assert.throws(() => buildWorldSimulationObserverTickSnapshotReadiness({
  ledger, chronological_mutation_queue: {
    ...exactQueue, batches: [...exactQueue.batches].reverse(),
  },
}), /modified authoritative mutation queue/u);

const fixtureRoot = path.join(projectRoot, "tests", ".tmp",
  `cc7f-observer-snapshot-readiness-${process.pid}-${Date.now()}`);
...
try {
  ...
  assert.equal(nativeReadiness.boundaries.private_or_future_state_exposed, false);
  assert(nativeReadiness.ticks.every((item) =>
    item.world_snapshot_materialized === false
    && item.character_view_created === false));
  assert.equal(JSON.stringify(nativeReadiness).includes(surface), false);
...
console.log("CC-7F observer tick snapshot readiness tests passed.");
