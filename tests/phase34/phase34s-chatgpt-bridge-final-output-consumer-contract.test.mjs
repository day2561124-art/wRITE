...
function assertConsumerLock(output, label) {
  assert.equal(output.consumer_must_emit_output_text_exactly, true, label + ": consumer exact emit mismatch.");
  assert.equal(output.consumer_output_source, "chatgpt_final_output.output_text", label + ": consumer output source mismatch.");
  assert.equal(output.consumer_may_read_result, false, label + ": consumer result read mismatch.");
  assert.equal(output.consumer_may_read_result_final_candidate_text, false, label + ": consumer final candidate read mismatch.");
  assert.equal(output.consumer_may_read_result_success_output, false, label + ": consumer success output read mismatch.");
  assert.equal(output.consumer_may_read_result_failure_output, false, label + ": consumer failure output read mismatch.");
  assert.equal(output.consumer_may_read_result_final_response, false, label + ": consumer final response read mismatch.");
  assert.equal(output.consumer_may_read_result_final_response_handoff, false, label + ": consumer handoff read mismatch.");
  assert.equal(output.consumer_may_read_result_extracted_chatgpt_final_output, false, label + ": consumer extracted read mismatch.");
  assert.equal(output.consumer_may_recompose_from_result, false, label + ": consumer recomposition mismatch.");
}
...
function assertConsumerEmission(toolResponse, expected, label) {
  const rootOutput = toolResponse.chatgpt_final_output;
  ...
  assert.equal(consumer.used, true, label + ": consumer used mismatch.");
  assert.equal(consumer.phase, "34S", label + ": consumer phase mismatch.");
  assert.equal(consumer.surface_kind, "chatgpt_bridge_final_output_consumer_contract", label + ": consumer surface kind mismatch.");
  assert.equal(consumer.contract_valid, true, label + ": consumer contract valid mismatch.");
  assert.deepEqual(consumer.validation_errors, [], label + ": consumer validation errors should be empty.");
  assert.equal(consumer.status, "ready_to_emit_consumer_final_output", label + ": consumer status mismatch.");
  assert.equal(consumer.response_kind, expected.responseKind, label + ": consumer response kind mismatch.");
  assert.equal(consumer.can_emit_response_to_chat, true, label + ": can emit mismatch.");
  assert.equal(consumer.output_source, "chatgpt_final_output.output_text", label + ": output source mismatch.");
  assert.equal(consumer.final_output_source, "chatgpt_final_output.output_text", label + ": final output source mismatch.");
  assert.equal(consumer.source, "chatgpt_final_output.output_text", label + ": source mismatch.");
  assert.equal(consumer.source_surface, "chatgpt_final_output", label + ": source surface mismatch.");
  assert.equal(consumer.source_response_kind, rootOutput.response_kind, label + ": source response kind mismatch.");
  assert.equal(consumer.source_status, rootOutput.status, label + ": source status mismatch.");
  assert.equal(consumer.source_output_hash, rootOutput.output_hash, label + ": source output hash mismatch.");
  assert.equal(consumer.source_output_source, rootOutput.output_source, label + ": source output source mismatch.");
  assert.equal(consumer.output_text, rootOutput.output_text, label + ": consumer must emit root output text exactly.");
  assert.equal(consumer.output_hash, rootOutput.output_hash, label + ": consumer hash must equal root hash.");
  assert.equal(consumer.output_hash, sha256(consumer.output_text), label + ": consumer hash mismatch.");
  assertNoFallback(consumer, label);
  assertConsumerLock(consumer, label);
  assertOutputSafety(consumer.safety, label);

  if (expected.responseKind === "final_candidate_text") {
    assert.equal(consumer.can_output_to_chat, true, label + ": success output gate mismatch.");
    assert.equal(consumer.may_output_story_text, true, label + ": success story output mismatch.");
    assert.equal(consumer.output_text, expected.finalCandidateText, label + ": success text mismatch.");
  } else {
    assert.equal(consumer.can_output_to_chat, false, label + ": failure output gate mismatch.");
    assert.equal(consumer.may_output_story_text, false, label + ": failure story output mismatch.");
    assert.equal(consumer.output_text.includes(expected.forbiddenStoryText), false, label + ": failure consumer must not include forbidden story text.");
  }
}

function assertInvalidConsumerContract() {
  const missing = buildChatgptFinalOutputConsumerEmission({});
  assert.equal(missing.used, true, "missing consumer: used mismatch.");
  assert.equal(missing.phase, "34S", "missing consumer: phase mismatch.");
  assert.equal(missing.contract_valid, false, "missing consumer should be invalid.");
  assert.equal(missing.response_kind, "consumer_contract_invalid_notice", "missing consumer response kind mismatch.");
  assert.equal(missing.can_emit_response_to_chat, true, "missing consumer can emit mismatch.");
  assert.equal(missing.can_output_to_chat, false, "missing consumer output gate mismatch.");
  assert.equal(missing.output_hash, sha256(missing.output_text), "missing consumer hash mismatch.");
  assert.equal(missing.validation_errors.includes("chatgpt_final_output_used_false_or_missing"), true, "missing consumer validation error missing.");
  assertNoFallback(missing, "missing consumer");
  assertConsumerLock(missing, "missing consumer");
  assertOutputSafety(missing.safety, "missing consumer");

  const forbiddenStoryText = "這段正文候選不可被 consumer invalid notice echo。";
  const invalid = buildChatgptFinalOutputConsumerEmission({
  ...
  assert.equal(invalid.used, true, "invalid consumer: used mismatch.");
  assert.equal(invalid.contract_valid, false, "invalid consumer should be invalid.");
  assert.equal(invalid.response_kind, "consumer_contract_invalid_notice", "invalid consumer response kind mismatch.");
  assert.equal(invalid.output_text.includes(forbiddenStoryText), false, "invalid consumer must not echo story text.");
  assert.equal(invalid.validation_errors.includes("chatgpt_final_output_contract_invalid"), true, "invalid consumer contract error missing.");
  assert.equal(invalid.validation_errors.includes("output_hash_mismatch"), true, "invalid consumer hash error missing.");
  assertNoFallback(invalid, "invalid consumer");
  assertConsumerLock(invalid, "invalid consumer");
  assertOutputSafety(invalid.safety, "invalid consumer");
}

function assertConsumerIgnoresResultFallbackSurfaces() {
  const rootText = "ROOT ONLY：ChatGPT 只能輸出這一段。";
  ...
  const consumer = buildChatgptFinalOutputConsumerEmission(toolResponse);
  assert.equal(consumer.contract_valid, true, "decoy consumer should be valid.");
  assert.equal(consumer.output_text, rootText, "consumer must emit only root output text.");

  for (const forbidden of forbiddenTexts) {
    assert.equal(consumer.output_text.includes(forbidden), false, "consumer output must ignore fallback text: " + forbidden);
  }

  assertNoFallback(consumer, "decoy consumer");
  assertConsumerLock(consumer, "decoy consumer");
}
...
const baseInput = {
  task_prompt: "Phase34S ChatGPT bridge final output consumer contract.",
  generation_context: {
    project: "武裝學院的二三事",
    chapter_mode: "candidate only",
    phase34s_final_output_consumer_contract: true,
...
function baseOptions(label, tempRoot, extra = {}) {
  return {
    ...
    characterMindStateLedger: {
      version: "phase34s-test-ledger-v1",
      ...
      characters: [
        {
          ...
          evidence_refs: ["phase34s-final-output-consumer"],
        },
        {
          character_name: "九逃",
...
const cases = [
  {
    ...
    optionsFor: (label, tempRoot) => baseOptions(label, tempRoot, {
      generationAdapter: async () => ({
        text: weakDraft,
        model_name: "deterministic-phase34s-final-output-consumer",
...
assertInvalidConsumerContract();
assertConsumerIgnoresResultFallbackSurfaces();

const protectedBefore = new Map();
for (const file of protectedFiles) protectedBefore.set(file, sha256(await readFile(file, "utf8")));
const transactionsBefore = await names(transactionDir);
const tempRoot = await mkdtemp(path.join(root, "data", "outputs", ".phase34s-"));
...
try {
  for (const item of cases) {
    ...
    assertConsumerEmission(bridge, item.expected, "bridge " + item.label);
  }
  ...
  assert(runAllText.includes(phase34r), "run-all missing Phase34R predecessor.");
  assert(runAllText.includes(phase34s), "run-all missing Phase34S registration.");
  assert(runAllText.includes(daily), "run-all missing Daily scripts registration.");
  assert(runAllText.indexOf(phase34r) < runAllText.indexOf(phase34s), "Phase34S should run after Phase34R.");
  assert(runAllText.indexOf(phase34s) < runAllText.indexOf(daily), "Phase34S should run before Daily scripts.");

  for (const file of protectedFiles) {
    assert.equal(sha256(await readFile(file, "utf8")), protectedBefore.get(file), file + " protected hash changed.");
  }

  console.log("Phase34S ChatGPT bridge final output consumer contract tests passed.");
} finally {
